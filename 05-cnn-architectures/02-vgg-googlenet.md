...
> 📊 **그림 1**: VGG16의 블록 구조 — 채널 2배 증가, 공간 절반 감소 패턴

```mermaid
flowchart LR
    A["입력<br/>224x224x3"] --> B["Block 1<br/>Conv3x3 x2<br/>112x112x64"]
    B --> C["Block 2<br/>Conv3x3 x2<br/>56x56x128"]
    C --> D["Block 3<br/>Conv3x3 x3<br/>28x28x256"]
    D --> E["Block 4<br/>Conv3x3 x3<br/>14x14x512"]
    E --> F["Block 5<br/>Conv3x3 x3<br/>7x7x512"]
    F --> G["FC 4096<br/>FC 4096<br/>FC 1000"]
```
...
> 📊 **그림 2**: Inception 모듈 — 4개 경로 병렬 처리 후 합침

```mermaid
flowchart TD
    INPUT["입력 특성 맵"] --> P1["경로 1<br/>1x1 Conv"]
    INPUT --> R2["1x1 Conv<br/>채널 축소"]
    INPUT --> R3["1x1 Conv<br/>채널 축소"]
    INPUT --> P4["3x3 MaxPool"]
    R2 --> P2["경로 2<br/>3x3 Conv"]
    R3 --> P3["경로 3<br/>5x5 Conv"]
    P4 --> P4B["경로 4<br/>1x1 Conv"]
    P1 --> CAT["채널 축 합침<br/>Concat"]
    P2 --> CAT
    P3 --> CAT
    P4B --> CAT
    CAT --> OUT["출력 특성 맵"]
...
> 📊 **그림 3**: 1x1 합성곱 병목 구조의 연산량 절감 효과

```mermaid
flowchart LR
    subgraph DIRECT["직접 연결: 6.4억 연산"]
        A1["256채널"] -->|"5x5 Conv"| B1["128채널"]
    end
    subgraph BOTTLENECK["병목 구조: 0.86억 연산"]
        A2["256채널"] -->|"1x1 Conv"| M2["32채널<br/>병목"]
        M2 -->|"5x5 Conv"| B2["128채널"]
    end
```
...
> 📊 **그림 4**: GoogLeNet 전체 구조 — 보조 분류기의 위치

```mermaid
flowchart TD
    IN["입력 이미지"] --> STEM["Stem 레이어<br/>Conv, Pool"]
    STEM --> INC1["Inception 3a, 3b"]
    INC1 --> INC2["Inception 4a"]
    INC2 --> AUX1["보조 분류기 1<br/>학습 시만 사용"]
    INC2 --> INC3["Inception 4b~4e"]
    INC3 --> AUX2["보조 분류기 2<br/>학습 시만 사용"]
    INC3 --> INC4["Inception 5a, 5b"]
    INC4 --> GAP["Global Average Pooling"]
    GAP --> OUT["최종 분류기<br/>1000 클래스"]
    style AUX1 stroke-dasharray: 5 5
    style AUX2 stroke-dasharray: 5 5
```
...
> 📊 **그림 5**: VGG vs GoogLeNet — 설계 철학 비교

```mermaid
flowchart LR
    subgraph VGG["VGG: 단순함의 힘"]
        direction TD
        V1["3x3 Conv"] --> V2["3x3 Conv"]
        V2 --> V3["3x3 Conv"]
        V3 --> V4["3x3 Conv"]
        V4 --> V5["...<br/>직선 구조"]
        V5 --> VFC["FC 레이어<br/>1.38억 파라미터"]
    end
    subgraph GOOG["GoogLeNet: 효율의 힘"]
        direction TD
        G1["Inception 모듈"] --> G2["Inception 모듈"]
        G2 --> G3["Inception 모듈"]
        G3 --> G4["...<br/>모듈 반복"]
        G4 --> GGAP["GAP<br/>700만 파라미터"]
    end
```
