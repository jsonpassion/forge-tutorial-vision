...
> 📊 **그림 1**: ResNet → ConvNeXt 단계별 현대화와 정확도 변화

```mermaid
flowchart TD
    A["ResNet-50<br/>76.1%"] --> B["Step 1: 학습 기법 현대화<br/>AdamW, 300 에포크, Mixup 등"]
    B --> C["78.8% (+2.7%)"]
    C --> D["Step 2: 매크로 설계<br/>블록 비율 (3,3,9,3)"]
    D --> E["79.4% (+0.6%)"]
    E --> F["Step 3: Depthwise Conv<br/>공간/채널 처리 분리"]
    F --> G["80.5% (+1.1%)"]
    G --> H["Step 4: 커널 7x7<br/>수용 영역 확대"]
    H --> I["80.6% (+0.1%)"]
    I --> J["Step 5: 마이크로 설계<br/>GELU, LayerNorm, 최소 활성화"]
    J --> K["ConvNeXt<br/>82.0% (+1.4%)"]
```
...
> 📊 **그림 2**: ConvNeXt 블록 vs Transformer 블록 구조 비교

```mermaid
flowchart LR
    subgraph ConvNeXt["ConvNeXt Block"]
        direction TB
        CA["입력"] --> CB["DWConv 7x7<br/>공간 처리"]
        CB --> CC["LayerNorm"]
        CC --> CD["Linear 1x1<br/>채널 4배 확장"]
        CD --> CE["GELU"]
        CE --> CF["Linear 1x1<br/>채널 축소"]
        CF --> CG["+ Skip Connection"]
    end
    subgraph Transformer["Transformer Block"]
        direction TB
        TA["입력"] --> TB["Self-Attention<br/>글로벌 처리"]
        TB --> TC["LayerNorm"]
        TC --> TD["FFN 확장<br/>채널 4배"]
        TD --> TE["GELU"]
        TE --> TF["FFN 축소<br/>채널 복원"]
        TF --> TG["+ Skip Connection"]
    end
```
...
> 📊 **그림 3**: CNN 아키텍처 진화 계보 — LeNet에서 ConvNeXt까지

```mermaid
graph TD
    A["LeNet<br/>1998"] --> B["AlexNet<br/>2012"]
    B --> C["VGG<br/>2014"]
    ...
    H["Vision Transformer<br/>2020"] -.->|설계 원리 차용| I
    E -->|현대화| I["ConvNeXt<br/>2022"]
    I --> J["ConvNeXt V2<br/>2023"]
    K["MAE 자기지도 학습"] -.->|FCMAE| J
```
