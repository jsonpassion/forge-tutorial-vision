...
> 📊 **그림 1**: DenseNet의 밀집 연결 구조 — 모든 레이어가 이전 레이어 전부와 연결
...
graph LR
    ...
    L2 -->|"x2"| L4
    L3 -->|"x3"| L4
    L4 --> OUT["출력<br/>(x0,x1,x2,x3,x4) concat"]
```
...
> 📊 **그림 2**: DenseNet 전체 아키텍처 — Dense Block과 Transition Layer의 반복

```mermaid
flowchart LR
    A["입력 이미지"] --> B["Conv + Pool"]
    B --> D1["Dense Block 1<br/>밀집 연결"]
    D1 --> T1["Transition 1<br/>1x1 Conv + AvgPool"]
    T1 --> D2["Dense Block 2<br/>밀집 연결"]
    D2 --> T2["Transition 2<br/>1x1 Conv + AvgPool"]
    T2 --> D3["Dense Block 3<br/>밀집 연결"]
    D3 --> T3["Transition 3"]
    T3 --> D4["Dense Block 4<br/>밀집 연결"]
    D4 --> G["GAP"]
    G --> FC["FC + Softmax"]
```
...
> 📊 **그림 3**: SE 블록의 Squeeze-Excitation 동작 흐름

```mermaid
flowchart TD
    IN["입력 특성맵<br/>C x H x W"] --> SQ["Squeeze<br/>Global Avg Pooling"]
    SQ --> V["채널 요약 벡터<br/>C x 1 x 1"]
    V --> FC1["FC: C -> C/r<br/>+ ReLU"]
    FC1 --> FC2["FC: C/r -> C<br/>+ Sigmoid"]
    FC2 --> W["채널 가중치<br/>(0~1 사이)"]
    IN --> MUL(("Scale<br/>곱하기"))
    W --> MUL
    MUL --> OUT["출력 특성맵<br/>C x H x W"]
```
...
> 📊 **그림 4**: SE 블록의 플러그인 적용 — ResNet Block에 SE 추가

```mermaid
flowchart TD
    X["입력 x"] --> CONV1["3x3 Conv + BN + ReLU"]
    CONV1 --> CONV2["3x3 Conv + BN"]
    CONV2 --> SE["SE 블록<br/>채널 어텐션"]
    X --> ADD(("+ 덧셈"))
    SE --> ADD
    ADD --> RELU["ReLU"]
    RELU --> OUT["출력"]
    style SE fill:#f9d,stroke:#c06,stroke-width:2px
```
