...
flowchart LR
    ...
    subgraph 복합["복합 스케일링"]
        C["깊이 + 너비 + 해상도<br/>균형 확장"] --> C1["효율적 성능 향상"]
    end
```
...
> 📊 **그림 2**: 복합 계수 phi에 의한 자동 스케일링 과정

```mermaid
flowchart TD
    PHI["복합 계수 phi"] --> D["깊이: alpha^phi"]
    PHI --> W["너비: beta^phi"]
    PHI --> R["해상도: gamma^phi"]
    D --> B0["B0<br/>224, 5.3M"]
    W --> B0
    R --> B0
    B0 -->|"phi 증가"| B4["B4<br/>380, 19M"]
    B4 -->|"phi 증가"| B7["B7<br/>600, 66M"]
```
...
> 📊 **그림 3**: MBConv 블록의 내부 구조

```mermaid
flowchart TD
    IN["입력"] --> EX["1x1 Conv<br/>채널 확장"]
    EX --> BN1["BatchNorm + Swish"]
    BN1 --> DW["Depthwise Conv 3x3/5x5<br/>공간 특성 추출"]
    DW --> BN2["BatchNorm + Swish"]
    BN2 --> SE["SE 블록<br/>채널 어텐션"]
    SE --> PJ["1x1 Conv<br/>채널 축소"]
    PJ --> BN3["BatchNorm"]
    BN3 --> ADD(("+ 잔차 연결"))
    IN -->|"Skip Connection"| ADD
    ADD --> OUT["출력"]
```
...
> 📊 **그림 4**: NAS 기반 EfficientNet 설계 과정

```mermaid
flowchart LR
    A["목표 설정<br/>정확도 + 효율성"] --> B["AutoML MNAS<br/>아키텍처 탐색"]
    B --> C["수천 개 후보<br/>자동 평가"]
    C --> D["B0 베이스라인<br/>최적 구조 발견"]
    D --> E["복합 스케일링<br/>적용"]
    E --> F["B1 ~ B7<br/>패밀리 생성"]
```
...
> 📊 **그림 5**: EfficientNetV2의 두 가지 핵심 개선

```mermaid
flowchart TD
    subgraph V1["EfficientNet V1"]
        A1["Depthwise Conv"] --> A2["1x1 Conv"]
        A3["고정 해상도로 학습"]
    end
    subgraph V2["EfficientNet V2"]
        B1["Fused-MBConv<br/>초기 레이어: 3x3 Conv 통합"] --> B2["MBConv<br/>후반 레이어"]
        B3["점진적 학습<br/>작은 이미지 -> 큰 이미지<br/>약한 정규화 -> 강한 정규화"]
    end
    V1 -->|"개선"| V2
    V2 --> R["학습 속도 5~11배 향상"]
```
