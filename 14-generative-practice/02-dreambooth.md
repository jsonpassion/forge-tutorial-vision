...
> 📊 **그림 1**: DreamBooth의 핵심 메커니즘 — 고유 식별자로 특정 주체를 모델에 각인

```mermaid
flowchart LR
    A["학습 이미지<br/>3~5장"] --> B["고유 식별자<br/>sks"]
    C["클래스 명사<br/>dog"] --> D["프롬프트 결합<br/>'sks dog'"]
    B --> D
    D --> E["전체 모델<br/>파인튜닝"]
    E --> F["개인화된 모델"]
    F --> G["'sks dog wearing<br/>a crown'"]
    G --> H["내 강아지가<br/>왕관 쓴 이미지"]
```
...
> 📊 **그림 2**: Prior Preservation Loss의 이중 학습 구조

```mermaid
flowchart TD
    subgraph 학습데이터
        A["주체 이미지<br/>'sks dog' = 내 강아지"] 
        B["클래스 이미지<br/>'dog' = 일반 개 200~500장"]
    end
    subgraph 손실함수
        C["Subject Loss<br/>주체 학습"]
        D["Prior Loss<br/>망각 방지"]
    end
    A --> C
    B --> D
    C --> E["총 손실<br/>L = L_subject + lambda * L_prior"]
    D --> E
    E --> F["모델 업데이트"]
    F --> G{"결과"}
    G --> H["'sks dog' = 내 강아지 O"]
...
> 📊 **그림 3**: DreamBooth, LoRA, DreamBooth+LoRA 비교

```mermaid
graph TD
    A["파인튜닝 기법"] --> B["DreamBooth"]
    A --> C["LoRA"]
    A --> D["DreamBooth + LoRA"]
    B --> B1["전체 가중치 수정<br/>2~8GB 결과물"]
    B --> B2["주체 충실도 최고<br/>얼굴, 캐릭터"]
    C --> C1["저랭크 행렬만 학습<br/>1~100MB 결과물"]
    C --> C2["스타일 학습 최적<br/>화풍, 색감"]
    D --> D1["DreamBooth 방식 학습<br/>LoRA 형태 저장"]
    D --> D2["높은 품질 + 효율성<br/>현재 표준"]
```
...
> 📊 **그림 4**: DreamBooth 학습 파이프라인 전체 흐름

```mermaid
flowchart LR
    A["주체 사진<br/>3~5장 준비"] --> B["클래스 이미지<br/>자동 생성 200장"]
    B --> C["데이터셋 구성<br/>주체 + 클래스"]
    C --> D["DreamBooth 학습<br/>500~800 스텝"]
    D --> E{"저장 방식"}
    E --> F["전체 모델<br/>2~8GB"]
    E --> G["LoRA 가중치<br/>수십 MB"]
    F --> H["추론<br/>'sks dog ...'"]
    G --> H
```
...
> 📊 **그림 5**: 개인화 기법별 수정 범위 비교

```mermaid
flowchart TD
    subgraph TI["Textual Inversion"]
        T1["텍스트 임베딩"] --> T2["새 토큰 벡터만<br/>학습 ~10KB"]
    end
    subgraph DB["DreamBooth"]
        D1["텍스트 인코더"] --> D2["U-Net 전체<br/>가중치 수정 ~2GB"]
    end
    subgraph DBL["DreamBooth + LoRA"]
        L1["U-Net 어텐션 레이어"] --> L2["저랭크 행렬만<br/>학습 ~수십MB"]
    end
    TI --> R1["충실도: 중간"]
    DB --> R2["충실도: 최고"]
    DBL --> R3["충실도: 높음 + 효율적"]
