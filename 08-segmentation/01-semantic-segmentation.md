...
> 📊 **그림 1**: FCN 아키텍처 — 분류 네트워크를 세그멘테이션으로 변환하는 흐름

```mermaid
flowchart LR
    A["입력 이미지<br/>H x W x 3"] --> B["합성곱 인코더<br/>(VGG16 등)"]
    B --> C["특징 맵<br/>H/32 x W/32"]
    C --> D["1x1 합성곱<br/>(FC 레이어 대체)"]
    D --> E["클래스 히트맵<br/>H/32 x W/32 x C"]
    E --> F["업샘플링<br/>(전치 합성곱)"]
    F --> G["세그멘테이션 맵<br/>H x W"]
```
...
> 📊 **그림 2**: FCN 변형 비교 — 스킵 연결을 통한 점진적 품질 향상

```mermaid
flowchart TD
    subgraph FCN32["FCN-32s"]
        A1["pool5"] --> U1["32x 업샘플"]
        U1 --> O1["거친 출력"]
    end
    subgraph FCN16["FCN-16s"]
        A2["pool5"] --> U2["2x 업샘플"]
        P4a["pool4"] --> ADD2["+"]
        U2 --> ADD2
        ADD2 --> U2b["16x 업샘플"]
...
> 📊 **그림 3**: U-Net의 U자형 아키텍처 — 인코더-디코더 + 스킵 연결

```mermaid
flowchart LR
    subgraph Encoder["인코더(수축 경로)"]
        E1["Conv 64<br/>H x W"] --> E2["Conv 128<br/>H/2 x W/2"]
        E2 --> E3["Conv 256<br/>H/4 x W/4"]
    end
    E3 --> BN["바틀넥<br/>Conv 512<br/>H/8 x W/8"]
    subgraph Decoder["디코더(확장 경로)"]
        D3["Conv 256<br/>H/4 x W/4"] --> D2["Conv 128<br/>H/2 x W/2"]
        D2 --> D1["Conv 64<br/>H x W"]
    end
    BN --> D3
    E3 -."스킵 연결(concat)".-> D3
    E2 -."스킵 연결(concat)".-> D2
    E1 -."스킵 연결(concat)".-> D1
    D1 --> OUT["1x1 Conv<br/>세그멘테이션 맵"]
```
...
> 📊 **그림 4**: ASPP 모듈 — 다양한 팽창률로 다중 스케일 문맥 포착

```mermaid
flowchart TD
    IN["입력 특징 맵"] --> B1["1x1 Conv<br/>(가까운 문맥)"]
    IN --> B2["3x3 Atrous<br/>r=6"]
    IN --> B3["3x3 Atrous<br/>r=12"]
    IN --> B4["3x3 Atrous<br/>r=18"]
    IN --> B5["Global Average<br/>Pooling"]
    B1 --> CAT["Concat"]
    B2 --> CAT
    B3 --> CAT
    B4 --> CAT
    B5 --> CAT
    CAT --> PROJ["1x1 Conv 투영"]
    PROJ --> OUT["ASPP 출력<br/>(256채널)"]
```
...
> 📊 **그림 5**: 세그멘테이션 손실 함수 선택 전략

```mermaid
flowchart TD
    Q{"클래스 불균형이<br/>심한가?"}
    Q -->|"아니오<br/>(균형 데이터)"| CE["Cross-Entropy Loss<br/>픽셀별 분류 손실"]
    Q -->|"예<br/>(배경 90%+)"| BOTH{"어떤 전략?"}
    BOTH --> WCE["Weighted CE<br/>클래스별 가중치 부여"]
    BOTH --> DICE["Dice Loss<br/>겹침 영역 직접 최적화"]
    BOTH --> COMB["CE + Dice 조합<br/>(실무 권장)"]
    WCE --> RESULT["학습 안정성 확보"]
    DICE --> RESULT
    COMB --> RESULT
```
