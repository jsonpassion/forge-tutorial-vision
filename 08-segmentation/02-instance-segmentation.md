...
> 📊 **그림 1**: 인스턴스 세그멘테이션의 두 가지 접근 방식

```mermaid
flowchart TD
    subgraph TD["Top-Down 방식"]
        A1["객체 탐지"] --> A2["바운딩 박스 추출"]
        A2 --> A3["박스 내 마스크 예측"]
    end
    subgraph BU["Bottom-Up 방식"]
        B1["픽셀 단위 세그멘테이션"] --> B2["임베딩 학습"]
        B2 --> B3["같은 객체 픽셀 그룹핑"]
    end
    TD --> R["인스턴스별 마스크 출력"]
    BU --> R
...
> 📊 **그림 2**: Mask R-CNN 파이프라인 구조

```mermaid
flowchart LR
    A["입력 이미지"] --> B["백본<br/>ResNet + FPN"]
    B --> C["RPN"]
    C --> D["RoI 후보"]
    B --> E["특징 맵"]
    D --> F["RoIAlign"]
...
flowchart TD
    ...
    B --> C["Protonet"]
    B --> D["탐지 헤드"]
    C --> E["k개 프로토타입 마스크<br/>H x W x k"]
    D --> F["객체별 혼합 계수<br/>N x k"]
    E --> G["행렬 곱셈"]
    F --> G
    G --> H["N개 인스턴스 마스크<br/>H x W x N"]
```
...
> 📊 **그림 4**: 인스턴스 세그멘테이션 모델의 발전 흐름

```mermaid
flowchart LR
    A["Mask R-CNN<br/>2017"] --> B["YOLACT<br/>2019"]
    B --> C["YOLACT++<br/>2019"]
    A --> D["SOLOv2<br/>2020"]
    A --> E["CondInst<br/>2020"]
    subgraph BoxBased["박스 기반"]
        A
        B
        C
    end
    subgraph BoxFree["박스 프리"]
        D
        E
    end
    BoxBased --> F["Mask2Former<br/>2022"]
    BoxFree --> F
```
