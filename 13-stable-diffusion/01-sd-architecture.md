...
> 📊 **그림 1**: Stable Diffusion 전체 파이프라인 — 텍스트에서 이미지까지

```mermaid
flowchart LR
    A["텍스트 입력"] --> B["CLIP 텍스트 인코더"]
    B --> C["텍스트 임베딩<br/>77 x 768"]
    D["랜덤 노이즈<br/>4 x 64 x 64"] --> E["U-Net<br/>반복 디노이징"]
    C --> E
    E --> F["정제된 잠재 표현<br/>4 x 64 x 64"]
    F --> G["VAE 디코더"]
    G --> H["최종 이미지<br/>3 x 512 x 512"]
```
...
> 📊 **그림 2**: 세 모듈의 역할과 데이터 흐름

```mermaid
graph TD
    subgraph CLIP["CLIP 텍스트 인코더<br/>1.23억 파라미터"]
        T["텍스트 토큰화"] --> TE["토큰 임베딩<br/>77 x 768"]
    end
    subgraph UNET["U-Net<br/>8.6억 파라미터"]
        SA["셀프 어텐션"] --- CA["크로스 어텐션"]
        CA --- RB["ResNet 블록"]
    end
    subgraph VAE_MOD["VAE<br/>0.84억 파라미터"]
        ENC["인코더<br/>이미지 -> 잠재"] --- DEC["디코더<br/>잠재 -> 이미지"]
    end
    TE --> CA
    RB --> DEC
```
...
> 📊 **그림 3**: 크로스 어텐션 메커니즘 — 텍스트와 이미지의 연결

```mermaid
sequenceDiagram
    participant T as 텍스트 임베딩
    participant CA as 크로스 어텐션
    participant F as 이미지 특징맵
    F->>CA: Query 생성<br/>각 위치별 질의
    T->>CA: Key, Value 생성<br/>토큰별 의미
    CA->>CA: 어텐션 가중치 계산<br/>위치-토큰 매칭
    CA->>F: 텍스트 조건 반영된<br/>특징맵 업데이트
```
...
> 📊 **그림 4**: img2img 파이프라인 흐름

```mermaid
flowchart LR
    A["입력 이미지"] --> B["VAE 인코더"]
    B --> C["잠재 표현"]
    C --> D["노이즈 추가<br/>strength 비율"]
    D --> E["U-Net 디노이징"]
    F["텍스트 조건"] --> E
    E --> G["VAE 디코더"]
    G --> H["출력 이미지"]
...
> 📊 **그림 5**: 파이프라인 4단계 내부 동작

```mermaid
stateDiagram-v2
    [*] --> 텍스트인코딩: 1단계
    텍스트인코딩: CLIP 인코딩<br/>텍스트 -> (1, 77, 768)
    텍스트인코딩 --> 노이즈생성: 2단계
    노이즈생성: 랜덤 잠재 노이즈<br/>(1, 4, 64, 64)
    노이즈생성 --> 반복디노이징: 3단계
    반복디노이징: U-Net x N 스텝<br/>노이즈 예측 + 제거
    반복디노이징 --> VAE디코딩: 4단계
    VAE디코딩: 잠재 -> 이미지<br/>(1, 3, 512, 512)
    VAE디코딩 --> [*]
```
