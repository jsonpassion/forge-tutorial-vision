...
> 📊 **그림 5**: FLUX 전체 아키텍처 개관

```mermaid
flowchart LR
    subgraph 인코딩["텍스트 인코딩"]
        P["프롬프트"] --> CL["CLIP"]
        P --> T5["T5-XXL"]
    end
    subgraph 잠재공간["Latent Space"]
        VAE_E["VAE 인코더"] --> L["노이즈 Latent"]
    end
    CL --> MMDIT["MM-DiT<br/>119억 파라미터<br/>57 Transformer 블록"]
    T5 --> MMDIT
    L --> MMDIT
    MMDIT -->|"Rectified Flow<br/>디노이징"| DL["디노이즈된 Latent"]
    DL --> VAE_D["VAE 디코더"]
    VAE_D --> IMG["생성 이미지"]
```
...
> 📊 **그림 1**: MM-DiT의 이중 스트림 + 단일 스트림 처리 흐름

```mermaid
flowchart TD
    A["텍스트 토큰"] --> B["이중 스트림 블록 x19"]
    C["이미지 토큰"] --> B
    B --> D["텍스트 투영"]
    B --> E["이미지 투영"]
    D --> F["통합 어텐션"]
    ...
    G --> I["토큰 결합"]
    H --> I
    I --> J["단일 스트림 블록 x38<br/>통합 가중치 처리"]
    J --> K["최종 이미지 출력"]
```
...
> 📊 **그림 2**: DDPM vs Rectified Flow — 샘플링 경로 비교

```mermaid
flowchart LR
    subgraph DDPM["DDPM 방식"]
        direction LR
        N1["순수 노이즈"] -->|"곡선 경로"| S1["스텝 1"]
        S1 -->|"방향 보정"| S2["스텝 2"]
        S2 -->|"..."| S3["스텝 N"]
        S3 --> I1["이미지"]
    end
...
> 📊 **그림 3**: FLUX 듀얼 텍스트 인코더 구조

```mermaid
flowchart TD
    P["텍스트 프롬프트"] --> CL["CLIP ViT-L/14<br/>4억 파라미터"]
    P --> T5["T5-XXL<br/>47억 파라미터"]
    CL --> E1["시각-언어 정렬<br/>임베딩"]
    T5 --> E2["풍부한 텍스트 이해<br/>임베딩"]
    E1 --> MM["MM-DiT"]
    E2 --> MM
    NI["노이즈 이미지<br/>Latent"] --> MM
    MM --> OUT["디노이즈된 이미지"]
```
...
> 📊 **그림 4**: FLUX.1 변형 간 디스틸레이션 관계

```mermaid
flowchart TD
    PRO["FLUX.1 Pro<br/>Teacher 모델<br/>25-50스텝, 최고 품질"] -->|"가이던스<br/>디스틸레이션"| DEV["FLUX.1 Dev<br/>20-30스텝<br/>CFG 불필요"]
    PRO -->|"적대적<br/>디스틸레이션"| SCH["FLUX.1 Schnell<br/>1-4스텝<br/>초고속 생성"]
    DEV -.->|"비상용<br/>라이선스"| R["연구/개인"]
    SCH -.->|"Apache 2.0"| O["오픈소스/상용"]
    PRO -.->|"상용 API"| C["상업 프로젝트"]
```
