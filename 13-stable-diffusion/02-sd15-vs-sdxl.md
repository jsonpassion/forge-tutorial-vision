...
> 📊 **그림 1**: SD 1.5 vs SDXL 아키텍처 비교

```mermaid
graph LR
    subgraph SD15["SD 1.5"]
        A1["CLIP ViT-L<br/>768차원"] --> B1["U-Net<br/>8.6억 파라미터"]
        B1 --> C1["VAE Decoder"]
        C1 --> D1["512 x 512"]
    end
    subgraph SDXL["SDXL"]
        A2["OpenCLIP ViT-G<br/>1280차원"] --> E2["Concat<br/>2048차원"]
        A3["CLIP ViT-L<br/>768차원"] --> E2
        E2 --> B2["U-Net<br/>26억 파라미터"]
        B2 --> C2["VAE Decoder"]
        C2 --> D2["1024 x 1024"]
    end
```
...
> 📊 **그림 2**: SDXL 듀얼 텍스트 인코더 동작 흐름

```mermaid
flowchart TD
    P["프롬프트 입력"] --> E1["OpenCLIP ViT-bigG"]
    P --> E2["CLIP ViT-L/14"]
    E1 --> V1["1280차원 임베딩"]
    E2 --> V2["768차원 임베딩"]
    V1 --> CC["Concat 결합"]
    V2 --> CC
    CC --> OUT["2048차원 텍스트 임베딩"]
    OUT --> UNET["U-Net Cross-Attention"]
```


### 개념 3: 리파이너 — 2단계 생성 파이프라인

> 📊 **그림 3**: SDXL Base + Refiner 2단계 파이프라인

```mermaid
flowchart LR
    N["노이즈"] --> B["Base 모델<br/>스텝 1~24 (80%)"]
    B --> |"중간 잠재 벡터"| R["Refiner 모델<br/>스텝 25~30 (20%)"]
    R --> V["VAE Decoder"]
    V --> I["최종 이미지<br/>1024 x 1024"]
...
> 📊 **그림 4**: Stable Diffusion 버전 진화 과정

```mermaid
flowchart LR
    A["SD 1.5<br/>CLIP ViT-L<br/>512px"] --> B["SD 2.0/2.1<br/>OpenCLIP<br/>768px"]
    B --> C["SDXL<br/>듀얼 인코더<br/>1024px"]
    C --> D["FLUX<br/>차세대 아키텍처"]
    B -.- X["커뮤니티 외면<br/>호환성 파괴"]
