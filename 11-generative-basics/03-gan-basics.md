...
> 📊 **그림 1**: GAN의 기본 구조 — 생성자와 판별자의 적대적 관계
...
> 📊 **그림 2**: 미니맥스 게임 — 생성자와 판별자의 상반된 목표

```mermaid
flowchart TD
    subgraph 판별자D["판별자 D의 목표: 최대화"]
        D1["D(x) 높이기"] --> D1G["진짜를 진짜로 판별"]
        D2["D(G(z)) 낮추기"] --> D2G["가짜를 가짜로 판별"]
    end
    subgraph 생성자G["생성자 G의 목표: 최소화"]
        G1["D(G(z)) 높이기"] --> G1G["가짜를 진짜로 속이기"]
    end
    판별자D -- "경쟁" --> 생성자G
    생성자G -- "경쟁" --> 판별자D
```
...
> 📊 **그림 3**: GAN 학습 루프 — 판별자와 생성자의 번갈아 업데이트

```mermaid
sequenceDiagram
    participant RD as 실제 데이터
    participant G as 생성자 G
    participant D as 판별자 D
    rect rgb(255, 230, 230)
        Note over RD, D: Step 1 - 판별자 학습
        RD->>D: 진짜 이미지 (레이블: 1)
        G->>D: 가짜 이미지 (레이블: 0)
        D->>D: 가중치 업데이트
    end
    rect rgb(230, 230, 255)
        Note over G, D: Step 2 - 생성자 학습
        G->>D: 가짜 이미지
        D-->>G: 그래디언트 전달
        Note over D: 가중치 고정
        G->>G: 가중치 업데이트
    end
    Note over G, D: 수만~수십만 번 반복
```
...
> 📊 **그림 4**: GAN의 주요 도전 과제와 원인

```mermaid
flowchart TD
    GAN["GAN 학습"] --> MC["모드 붕괴<br/>Mode Collapse"]
    GAN --> TI["학습 불안정<br/>Training Instability"]
    GAN --> EV["평가의 어려움"]
    MC --> MC1["데이터 다양성 상실<br/>일부 모드만 학습"]
    TI --> TI1["D가 너무 강함<br/>G에 그래디언트 전달 X"]
    TI --> TI2["G가 너무 강함<br/>D가 학습 동기 상실"]
    EV --> EV1["FID, IS 등<br/>간접 지표 사용"]
    style MC fill:#ff9999,color:#333
    style TI fill:#ffcc99,color:#333
    style EV fill:#99ccff,color:#333
```
...
> 📊 **그림 5**: GAN 학습 진행에 따른 내시 균형 수렴 과정

```mermaid
stateDiagram-v2
    [*] --> 초기: 학습 시작
    초기: G - 랜덤 노이즈 출력<br/>D - 쉽게 구별 가능
    초기 --> 중기: 반복 학습
    중기: G - 점점 사실적<br/>D - 구별이 어려워짐
    중기 --> 수렴: 균형 도달
    수렴: G의 분포 = 실제 분포<br/>D(x) = 0.5
    수렴 --> [*]: 내시 균형
    중기 --> 붕괴: 균형 실패
    붕괴: 모드 붕괴 또는<br/>학습 불안정
    붕괴 --> 중기: 하이퍼파라미터 조정
```
