...
> 📊 **그림 1**: 판별 모델과 생성 모델의 데이터 흐름 비교

```mermaid
flowchart LR
    subgraph 판별모델["판별 모델"]
        A1["이미지 입력"] --> B1["특징 추출"] --> C1["클래스 예측<br/>P(Y|X)"]
    end
    subgraph 생성모델["생성 모델"]
        A2["랜덤 노이즈 z"] --> B2["분포 학습"] --> C2["새 이미지 생성<br/>P(X)"]
    end
```
...
> 📊 **그림 2**: 생성 모델의 학습과 샘플링 과정

```mermaid
flowchart TD
    A["학습 데이터<br/>실제 이미지들"] --> B["분포 학습<br/>p_theta(x) 근사"]
    B --> C["학습된 확률 분포"]
    C --> D["샘플링"]
    D --> E["새로운 이미지 생성"]
    F["목표: p_theta(x) = p_data(x)"] -.-> B
```
...
> 📊 **그림 3**: 세 가지 생성 모델의 핵심 전략 비교

```mermaid
flowchart TD
    ROOT["생성 모델"] --> VAE["VAE<br/>압축과 복원"]
    ROOT --> GAN["GAN<br/>적대적 경쟁"]
    ROOT --> DIFF["Diffusion<br/>노이즈 제거"]
    VAE --> V1["인코더: 이미지 -> 잠재 벡터"]
    V1 --> V2["디코더: 잠재 벡터 -> 이미지"]
    GAN --> G1["생성자: 가짜 이미지 생성"]
    GAN --> G2["판별자: 진짜/가짜 구별"]
    G1 <-->|"경쟁"| G2
    DIFF --> D1["순방향: 노이즈 추가"]
    D1 --> D2["역방향: 노이즈 제거"]
...
```mermaid
flowchart LR
    A["2013<br/>VAE"] --> B["2014<br/>GAN"]
    B --> C["2015<br/>DCGAN"]
    C --> D["2017<br/>Pix2Pix<br/>CycleGAN"]
    D --> E["2019<br/>StyleGAN2"]
    E --> F["2020<br/>DDPM"]
    F --> G["2022<br/>Stable Diffusion"]
    G --> H["2023-24<br/>SDXL, FLUX"]
```
