...
> 📊 **그림 1**: 오토인코더의 기본 구조 — 인코더가 입력을 압축하고 디코더가 복원합니다

```mermaid
flowchart LR
    A["입력 이미지<br/>28x28 = 784"] --> B["인코더<br/>Encoder"]
    B --> C["잠재 벡터 z<br/>저차원"]
    C --> D["디코더<br/>Decoder"]
    D --> E["복원 이미지<br/>28x28 = 784"]
    A -. "재구성 손실<br/>최소화" .-> E
```
...
> 📊 **그림 2**: 일반 AE vs VAE — 점 인코딩에서 분포 인코딩으로의 전환

```mermaid
flowchart TD
    subgraph AE["일반 오토인코더"]
        A1["이미지"] --> B1["인코더"]
        B1 --> C1["고정된 점 z"]
        C1 --> D1["디코더"]
    end
    subgraph VAE["Variational Autoencoder"]
        ...
        B2 --> M["평균 mu"]
        B2 --> S["분산 sigma"]
        M --> R["Reparameterization<br/>z = mu + sigma * eps"]
        S --> R
        R --> D2["디코더"]
    end
```
...
> 📊 **그림 3**: VAE 손실 함수의 두 구성 요소와 각각의 역할

```mermaid
flowchart TD
    L["VAE 총 손실"] --> R["재구성 손실<br/>Reconstruction Loss"]
    L --> K["KL 다이버전스<br/>KL Divergence"]
    R --> R1["입력과 복원의 차이 측정"]
    R --> R2["MSE 또는 BCE 사용"]
    R1 --> G1["목표: 원본을 잘 복원"]
    K --> K1["잠재 분포와<br/>표준 정규분포의 거리"]
    K --> K2["잠재 공간 정규화"]
    K1 --> G2["목표: 매끄러운 잠재 공간"]
```
...
> 📊 **그림 4**: 잠재 공간에서의 보간과 산술 연산

```mermaid
flowchart LR
    subgraph INTERP["보간 Interpolation"]
        Z1["웃는 얼굴<br/>잠재 벡터"] --> M1["alpha = 0.5<br/>중간 지점"]
        Z2["무표정<br/>잠재 벡터"] --> M1
        M1 --> R1["살짝 미소<br/>짓는 얼굴"]
    end
    subgraph ARITH["산술 연산"]
        A1["안경 쓴 남자"] --> OP["-남자 +여자"]
        OP --> A2["안경 쓴 여자"]
    end
```
...
> 📊 **그림 5**: VAE에서 파생된 주요 변형 모델의 계보

```mermaid
flowchart TD
    VAE["VAE<br/>2013, Kingma"] --> BVAE["beta-VAE<br/>2017"]
    VAE --> VQVAE["VQ-VAE<br/>2017"]
    BVAE --> BVAE1["KL 가중치 조절<br/>잠재 분리 강화"]
    VQVAE --> VQVAE2["VQ-VAE-2<br/>2019"]
    VQVAE2 --> VQVAE2D["다중 해상도 코드북<br/>고해상도 생성"]
    VQVAE --> SD["Stable Diffusion<br/>잠재 공간 압축에 활용"]
```
