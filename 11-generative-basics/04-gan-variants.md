...
```mermaid
graph TD
    A["기본 GAN<br/>2014"] --> B["cGAN<br/>조건부 생성"]
    A --> C["DCGAN<br/>2015, CNN 도입"]
    A --> W["WGAN<br/>2017, 안정적 손실"]
    C --> S["StyleGAN<br/>2018, 스타일 기반"]
    B --> P["Pix2Pix<br/>2016, 쌍 있는 변환"]
    B --> CY["CycleGAN<br/>2017, 쌍 없는 변환"]
    S --> S2["StyleGAN2/3<br/>2020-2021"]
```
...
> 📊 **그림 2**: DCGAN 생성자와 판별자의 대칭 구조

```mermaid
flowchart LR
    subgraph 생성자
        direction LR
        Z["잠재 벡터 z<br/>(100, 1, 1)"] --> G1["전치 합성곱<br/>+ BatchNorm<br/>+ ReLU"]
        G1 --> G2["전치 합성곱<br/>+ BatchNorm<br/>+ ReLU"]
        G2 --> G3["전치 합성곱<br/>+ Tanh"]
        G3 --> IMG["생성 이미지<br/>(1, 28, 28)"]
    end
    subgraph 판별자
        direction LR
        IMG2["입력 이미지<br/>(1, 28, 28)"] --> D1["스트라이드 합성곱<br/>+ LeakyReLU"]
        D1 --> D2["스트라이드 합성곱<br/>+ BatchNorm<br/>+ LeakyReLU"]
        D2 --> D3["합성곱<br/>+ Sigmoid"]
        D3 --> OUT["진짜/가짜<br/>(1)"]    
    end
```
...
> 📊 **그림 5**: Conditional GAN의 조건부 생성 구조

```mermaid
flowchart TD
    Z["노이즈 z"] --> CONCAT1["결합"]
    C1["조건 c<br/>(예: 숫자 7)"] --> CONCAT1
    CONCAT1 --> G["생성자 G"]
    G --> FAKE["생성 이미지"]
    FAKE --> CONCAT2["결합"]
    C2["조건 c"] --> CONCAT2
...
> 📊 **그림 3**: StyleGAN의 스타일 기반 생성 구조

```mermaid
flowchart TD
    Z["잠재 벡터 z"] --> MAP["매핑 네트워크<br/>(8층 MLP)"]
    MAP --> W["스타일 벡터 w"]
    CONST["상수 입력<br/>(4x4)"] --> SB1["합성 블록 1<br/>거친 스타일"]
    W --> |AdaIN| SB1
    N1["노이즈"] --> SB1
    SB1 --> SB2["합성 블록 2<br/>중간 스타일"]
    W --> |AdaIN| SB2
    N2["노이즈"] --> SB2
    SB2 --> SB3["합성 블록 3<br/>미세 스타일"]
    W --> |AdaIN| SB3
    N3["노이즈"] --> SB3
    SB3 --> OUT["최종 이미지<br/>(1024x1024)"]
```
...
> 📊 **그림 4**: CycleGAN의 순환 일관성 구조

```mermaid
flowchart LR
    subgraph 정방향 순환
        direction LR
        H1["말 이미지"] --> G1["G: 말 to 얼룩말"]
        G1 --> Z1["가짜 얼룩말"]
        Z1 --> F1["F: 얼룩말 to 말"]
        F1 --> H2["복원된 말"]
    end
