...
> 📊 **그림 4**: 실제 장면에서 NumPy 배열까지 — 이미지 디지털화 과정
...
flowchart LR
    ...
    B -->|"광전 변환"| C["아날로그 신호"]
    C -->|"AD 변환"| D["디지털 데이터"]
    D -->|"파일 저장"| E["이미지 파일<br/>JPG, PNG"]
    E -->|"Python 로드"| F["NumPy 배열<br/>(H, W, C)"]
```


## 핵심 개념

> 📊 **그림 1**: 디지털 이미지를 구성하는 세 가지 핵심 요소

```mermaid
graph TD
    A["디지털 이미지"] --> B["픽셀<br/>Pixel"]
    A --> C["해상도<br/>Resolution"]
    A --> D["비트 깊이<br/>Bit Depth"]
    B --> B1["이미지의 최소 단위<br/>색상 값을 담는 점"]
    C --> C1["가로 x 세로 픽셀 수<br/>선명도 결정"]
    D --> D1["채널당 표현 단계 수<br/>색 정밀도 결정"]
```
...
> 📊 **그림 2**: 해상도와 품질-비용의 트레이드오프

```mermaid
flowchart LR
    A["저해상도<br/>640x480"] --> B["중해상도<br/>1920x1080"]
    B --> C["고해상도<br/>3840x2160"]
    C --> D["초고해상도<br/>7680x4320"]
    A -.- A1["빠른 처리<br/>작은 용량"]
    D -.- D1["느린 처리<br/>큰 용량"]
...
> 📊 **그림 3**: 흑백 이미지와 컬러 이미지의 배열 구조 비교

```mermaid
flowchart TD
    subgraph GRAY["흑백 이미지"]
        G1["2D 배열<br/>(H x W)"] --> G2["밝기 값 1개<br/>0~255"]
    end
    subgraph COLOR["컬러 이미지 (RGB)"]
        C1["3D 배열<br/>(H x W x 3)"] --> R["R 채널<br/>빨강 밝기"]
        C1 --> G["G 채널<br/>초록 밝기"]
        C1 --> B["B 채널<br/>파랑 밝기"]
    end
    subgraph ALPHA["투명 포함 (RGBA)"]
        A1["3D 배열<br/>(H x W x 4)"] --> A2["RGB + A 채널<br/>투명도"]
    end
```
