...
> 📊 **그림 1**: 주요 색상 공간과 각각이 강조하는 측면

```mermaid
graph TD
    COLOR["하나의 색"] --> RGB["RGB<br/>빛의 삼원색"]
    COLOR --> HSV["HSV<br/>색상 + 채도 + 명도"]
    COLOR --> LAB["LAB<br/>인간 시각 모방"]
    COLOR --> GRAY["Grayscale<br/>밝기만"]
    RGB --> R_USE["디스플레이 표시<br/>딥러닝 입력"]
    HSV --> H_USE["색 기반 객체 검출<br/>색상 필터링"]
    LAB --> L_USE["색 보정<br/>조명 보정"]
    GRAY --> G_USE["에지 검출<br/>특징 추출"]
```
...
> 📊 **그림 2**: RGB vs HSV — 밝기 변화에 대한 대응 차이

```mermaid
flowchart LR
    subgraph RGB공간["RGB 공간"]
        A1["밝은 빨강<br/>R:255 G:50 B:50"] -.->|"값이 크게 다름"| A2["어두운 빨강<br/>R:120 G:20 B:20"]
    end
    subgraph HSV공간["HSV 공간"]
        B1["밝은 빨강<br/>H:0 S:200 V:255"] -->|"H는 동일!"| B2["어두운 빨강<br/>H:0 S:200 V:120"]
    end
    RGB공간 -->|"변환"| HSV공간
```
...
> 📊 **그림 3**: HSV 색상 환 — H 값에 따른 색상 배치 (OpenCV 기준 0~179)

```mermaid
flowchart LR
    R1["빨강<br/>H: 0~10"] --> O["주황<br/>H: 11~25"]
    O --> Y["노랑<br/>H: 26~34"]
    Y --> G["초록<br/>H: 35~85"]
    G --> B["파랑<br/>H: 86~125"]
    B --> P["보라<br/>H: 126~155"]
    P --> R2["빨강<br/>H: 156~179"]
    R2 -.->|"순환!"| R1
```
...
> 📊 **그림 4**: HSV 기반 색상 추출 파이프라인

```mermaid
flowchart TD
    A["원본 이미지<br/>BGR"] --> B["BGR to HSV 변환"]
    B --> C["H, S, V 범위 정의<br/>예: 빨강 H=0~10"]
    C --> D["inRange로<br/>마스크 생성"]
    B --> C2["두 번째 범위<br/>H=170~179"]
    C2 --> D2["두 번째<br/>마스크 생성"]
    D --> E["마스크 합치기<br/>mask1 + mask2"]
    D2 --> E
    E --> F["bitwise_and<br/>원본에 마스크 적용"]
    F --> G["색상 추출 결과"]
```
...
> 📊 **그림 5**: OpenCV BGR 순서와 라이브러리별 채널 순서 차이

```mermaid
sequenceDiagram
    participant FILE as 이미지 파일
    participant CV as OpenCV
    participant NP as NumPy 배열
    participant PLT as Matplotlib
    FILE->>CV: cv2.imread()
    CV->>NP: BGR 순서로 저장
    Note over NP: B=채널0, G=채널1, R=채널2
    NP->>CV: cv2.cvtColor(BGR2RGB)
    CV->>PLT: RGB 순서로 전달
    Note over PLT: R=채널0, G=채널1, B=채널2
    PLT->>PLT: 올바른 색상으로 표시
```
