...
> 📊 **그림 1**: 손실 압축과 무손실 압축의 차이

```mermaid
flowchart LR
    A["원본 이미지<br/>5.9 MB"] --> B{"압축 방식"}
    B -->|"손실 압축"| C["JPEG 저장<br/>0.5 MB"]
    B -->|"무손실 압축"| D["PNG 저장<br/>2.5 MB"]
    C -->|"복원"| E["근사 이미지<br/>일부 정보 손실"]
    D -->|"복원"| F["원본과 동일<br/>완벽 복원"]
```
...
> 📊 **그림 2**: JPEG 압축 파이프라인 — 5단계 처리 흐름

```mermaid
flowchart TD
    A["RGB 이미지"] --> B["1. 색상 변환<br/>RGB to YCbCr"]
    B --> C["2. 다운샘플링<br/>Cb, Cr 해상도 절반"]
    C --> D["3. DCT 변환<br/>8x8 블록 주파수 분해"]
    D --> E["4. 양자화<br/>고주파 성분 제거"]
    E --> F["5. 엔트로피 코딩<br/>허프만/산술 압축"]
    F --> G["JPEG 파일"]
    style E fill:#ff6b6b,color:#fff
    E -.->|"손실 발생 지점"| H["버려진 고주파 정보"]
```
...
```mermaid
graph TD
    ROOT["이미지 형식"] --> LOSSY["손실 압축"]
    ROOT --> LOSSLESS["무손실 압축"]
    ROOT --> NONE["비압축"]
    ROOT --> BOTH["손실 + 무손실"]
    LOSSY --> JPEG["JPEG<br/>사진 표준"]
    LOSSLESS --> PNG["PNG<br/>투명도 지원"]
    LOSSLESS --> GIF["GIF<br/>256색, 애니메이션"]
    LOSSLESS --> TIFF["TIFF<br/>의료, 인쇄"]
    NONE --> BMP["BMP<br/>윈도우 기본"]
    BOTH --> WEBP["WebP<br/>Google, 차세대"]
    BOTH --> AVIF["AVIF<br/>최신, 최고 압축률"]
```
...
> 📊 **그림 4**: CV 프로젝트 이미지 형식 선택 의사결정 트리

```mermaid
flowchart TD
    START["이미지 저장이 필요하다"] --> Q1{"품질 손실 허용?"}
    Q1 -->|"허용 불가"| Q2{"투명 배경 필요?"}
    Q1 -->|"허용"| Q3{"용도는?"}
    Q2 -->|"필요"| PNG["PNG"]
    Q2 -->|"불필요"| Q4{"중간 처리용?"}
    Q4 -->|"맞음"| NPY["NumPy .npy"]
