...
> 📊 **그림 1**: 활성화 함수 유무에 따른 레이어 효과 비교

```mermaid
flowchart TD
    subgraph NO["활성화 함수 없음"]
        A1["입력 x"] --> B1["레이어 1<br/>W1 x + b1"]
        B1 --> C1["레이어 2<br/>W2 y + b2"]
        C1 --> D1["레이어 3<br/>W3 z + b3"]
        D1 --> E1["결과 = Ax + c<br/>1개 레이어와 동일"]
    end
    subgraph YES["활성화 함수 있음"]
        A2["입력 x"] --> B2["레이어 1 + ReLU"]
        B2 --> C2["레이어 2 + ReLU"]
        C2 --> D2["레이어 3 + ReLU"]
        D2 --> E2["복잡한 비선형 함수<br/>깊을수록 표현력 증가"]
    end
```
...
> 📊 **그림 2**: ReLU 뉴런의 생명주기 — Dead Neuron 발생 과정

```mermaid
stateDiagram-v2
    [*] --> 정상: 초기화
    정상 --> 정상: 양수 입력<br/>기울기 = 1
    정상 --> 위험: 큰 음수 입력 반복
    위험 --> 정상: 가중치 업데이트로 회복
    위험 --> Dead: 가중치가 음수 고정
    Dead --> Dead: 출력 항상 0<br/>기울기 = 0<br/>학습 불가
    note right of Dead: Leaky ReLU로 방지 가능
```
...
flowchart TD
    ...
    Q2 -->|"회귀"| S3["없음 (Linear)"]
    Q3 -->|"CNN"| R1["ReLU"]
    Q3 -->|"Transformer"| G1["GELU"]
    Q3 -->|"RNN"| T1["Tanh"]
    R1 -->|"Dead Neuron 발생?"| L1["Leaky ReLU로 교체"]
```
