...
> 📊 **그림 3**: 순전파 과정 — 각 레이어에서 일어나는 연산

```mermaid
flowchart TD
    A["입력 데이터<br/>(784차원)"] --> B["Layer 1: Linear(784, 128)"]
    B --> B2["ReLU 활성화"]
    B2 --> C["Layer 2: Linear(128, 64)"]
    C --> C2["ReLU 활성화"]
    C2 --> D["Layer 3: Linear(64, 10)"]
    D --> E["출력<br/>(10개 클래스 점수)"]
```
...
```mermaid
flowchart LR
    A["1958<br/>퍼셉트론 발명<br/>로젠블랫"] --> B["1969<br/>민스키의 비판<br/>XOR 불가 증명"]
    B --> C["1970-1985<br/>AI 겨울"]
    C --> D["1986<br/>역전파 발표<br/>힌튼 등"]
    D --> E["2012~<br/>딥러닝 르네상스"]
    style C fill:#4a6fa5,stroke:#333,color:#fff
```
