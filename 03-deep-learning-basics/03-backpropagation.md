...
> 📊 **그림 1**: 신경망 학습의 전체 사이클 — 순전파부터 가중치 업데이트까지

```mermaid
flowchart LR
    A["입력 데이터"] --> B["순전파<br/>Forward Pass"]
    B --> C["예측값"]
    C --> D["손실 계산<br/>Loss"]
    D --> E["역전파<br/>Backward Pass"]
    E --> F["기울기 계산"]
    F --> G["가중치 업데이트"]
    G -->|"반복"| A
```
...
> 📊 **그림 2**: 경사하강법의 동작 원리 — 기울기 반대 방향으로 이동

```mermaid
flowchart TD
    A["현재 위치<br/>손실 = 높음"] -->|"기울기 계산"| B{"경사 방향은?"}
    B -->|"내리막 방향으로 이동"| C["학습률만큼 한 걸음"]
    C --> D["새 위치<br/>손실 = 감소"]
    D -->|"반복"| E{"충분히 낮은가?"}
    E -->|"아니오"| A
    E -->|"예"| F["최적 가중치 도달"]
...
> 📊 **그림 3**: 연쇄 법칙을 통한 역방향 기울기 전파

```mermaid
sequenceDiagram
    participant L as 손실(Loss)
    participant H2 as 출력층
    participant H1 as 은닉층 2
    participant H0 as 은닉층 1
    participant I as 입력층
    Note over L,I: 순전파 방향 →
    L->>H2: dL/dH2 (출력 기울기)
    H2->>H1: dL/dH1 = dL/dH2 x dH2/dH1
    H1->>H0: dL/dH0 = dL/dH1 x dH1/dH0
    H0->>I: dL/dI = dL/dH0 x dH0/dI
    Note over L,I: ← 역전파 방향 (기울기 곱셈 전파)
```
...
> 📊 **그림 4**: PyTorch Autograd의 계산 그래프 — 연산을 기록하고 역방향으로 미분

```mermaid
graph TD
    X["x 입력"] --> MUL["곱셈<br/>x * w"]
    W["w 가중치"] --> MUL
    MUL --> ADD["덧셈<br/>+ b"]
    B["b 편향"] --> ADD
    ADD --> PRED["예측값"]
    PRED --> LOSS["손실 계산<br/>(pred - target)²"]
    T["target 정답"] --> LOSS
    LOSS -->|".backward()"| GRAD["모든 기울기<br/>자동 계산"]
    style LOSS fill:#f96,stroke:#333
    style GRAD fill:#6f9,stroke:#333
```
...
> 📊 **그림 5**: 기울기 소실과 기울기 폭발 — 깊은 신경망의 두 가지 문제

```mermaid
flowchart LR
    subgraph 기울기 소실
        direction LR
        A1["출력층<br/>기울기 = 1.0"] --> A2["은닉층3<br/>x 0.1"]
        A2 --> A3["은닉층2<br/>x 0.01"]
        A3 --> A4["은닉층1<br/>x 0.001"]
        A4 --> A5["입력층<br/>거의 0"]
    end
    subgraph 기울기 폭발
        direction LR
        B1["출력층<br/>기울기 = 1.0"] --> B2["은닉층3<br/>x 10"]
        B2 --> B3["은닉층2<br/>x 100"]
        B3 --> B4["은닉층1<br/>x 1000"]
        B4 --> B5["입력층<br/>NaN/Inf"]
    end
```
...
### 역전파는 누가 발명했을까? — 최소 5명의 발명자

> 💡 **알고 계셨나요?**: 역전파의 "발명자"가 누구인지에 대해서는 꽤 복잡한 역사가 있습니다.

**1970년 — 세포 린나인마(Seppo Linnainmaa), 핀란드**

핀란드 헬싱키 대학의 석사 논문에서 **역방향 자동 미분(Reverse-mode automatic differentiation)**을 제안했습니다. 수학적으로 보면 이것이 역전파의 핵심 알고리즘과 동일합니다. 하지만 이 논문은 핀란드어로 쓰여 있었고, 신경망이 아닌 일반 수치 계산을 위한 것이었기에 AI 커뮤니티에서는 거의 알려지지 않았습니다.

**1974년 — 폴 워보스(Paul Werbos), 미국**

하버드 대학 박사 논문에서 역전파를 신경망 학습에 적용하는 방법을 제안했습니다. 하지만 당시는 AI 겨울의 한가운데였고, 신경망에 대한 관심이 바닥이었기에 **출판하는 데만 수년이 걸렸습니다.** 워보스는 이후 "아무도 내 논문을 읽지 않았다"고 회고하기도 했죠.

**1986년 — 루멜하트, 힌튼, 윌리엄스**

이 세 사람이 Nature에 발표한 *"Learning representations by back-propagating errors"* 논문이 역전파를 **세계적으로 유명하게** 만들었습니다. 핵심 알고리즘 자체가 새로운 것은 아니었지만, 다층 신경망에서 역전파가 실제로 잘 작동한다는 것을 명확히 보여준 것이 혁명적이었습니다.

제프리 힌튼(Geoffrey Hinton) 본인도 이렇게 말한 바 있습니다: **"I have never claimed that I invented backpropagation."** (나는 역전파를 발명했다고 주장한 적이 없다.) 정직하면서도 겸손한 태도인데, 실제로 역전파는 여러 사람이 독립적으로 발견한 아이디어입니다. 과학사에서는 이런 일이 종종 일어나죠 — 때가 무르익으면 여러 곳에서 동시에 같은 아이디어가 떠오르는 것입니다.

## 흔한 오해와 팁

> ⚠️ **흔한 오해**: "Hinton이 역전파를 발명했다"
>
> 위에서 살펴본 것처럼, Hinton 본인이 이를 부정합니다. 린나인마(1970), 워보스(1974) 등 여러 사람이 독립적으로 발견했으며, Hinton의 공헌은 역전파를 **대중화하고 실용적으로 증명**한 것입니다. 인용을 할 때는 "Rumelhart, Hinton & Williams (1986)"이 역전파를 널리 알린 논문이라고 하는 것이 정확합니다.

> ⚠️ **흔한 오해**: "역전파는 신경망 전용 알고리즘이다"
>
> 역전파의 본질은 **역방향 자동 미분(Reverse-mode Automatic Differentiation)**입니다. 신경망에 국한된 것이 아니라, 임의의 미분 가능한 연산에 적용할 수 있어요. PyTorch의 `autograd`가 `nn.Module` 밖에서도 일반 텐서 연산의 기울기를 계산할 수 있는 이유가 바로 이것입니다. 물리 시뮬레이션, 로봇 제어, 금융 모델링 등 다양한 분야에서도 역방향 자동 미분이 활용됩니다.

> 💡 **알고 계셨나요?**: 역전파가 효율적인 진짜 이유
>
> N개의 파라미터에 대한 기울기를 구해야 한다고 해봅시다. 각 파라미터마다 개별적으로 "이 파라미터를 살짝 바꾸면 출력이 얼마나 변할까?"를 계산하면 **N번의 순전파**가 필요하여 O(N^2) 시간이 걸립니다. 하지만 역전파는 **단 한 번의 역방향 패스**로 모든 파라미터의 기울기를 동시에 O(N) 시간에 계산합니다. 파라미터가 수백만~수십억 개인 현대 딥러닝에서, 이 차이는 "계산 가능 vs 불가능"의 차이입니다.
