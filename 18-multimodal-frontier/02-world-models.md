...
> 📊 **그림 1**: World Model의 핵심 루프 — 관찰, 예측, 행동, 피드백

```mermaid
flowchart LR
    A["현재 상태 s_t"] --> B["World Model f"]
    C["행동 a_t"] --> B
    B --> D["예측 상태 s_t+1"]
    D --> E{"계획 수립"}
    E -->|"최적 행동 선택"| C
...
> 📊 **그림 2**: 세 가지 World Model 아키텍처 패러다임 비교

```mermaid
flowchart TD
    subgraph R["재구성 기반<br/>Dreamer"]
        R1["관찰"] --> R2["인코더"]
        R2 --> R3["잠재 상태"]
        R3 --> R4["동역학 모델"]
        R4 --> R5["디코더"]
    ...
    subgraph P["예측 기반<br/>V-JEPA"]
        P1["관찰"] --> P2["인코더"]
        P2 --> P3["잠재 상태"]
        P3 --> P4["예측기"]
        P4 --> P5["미래 잠재"]
    end
    subgraph D["Diffusion 기반<br/>Sora"]
        D1["관찰 + 행동"] --> D2["노이즈 추가"]
        D2 --> D3["Diffusion 모델"]
        D3 --> D4["미래 관찰"]
    end
...
> 📊 **그림 3**: 비디오 생성 모델의 물리 이해 스펙트럼

```mermaid
graph LR
    A["패턴 매칭<br/>통계적 상관관계"] -->|"현재 수준"| B["근사적 물리<br/>Sora, Cosmos"]
    B -->|"목표"| C["인과적 이해<br/>물리 법칙 내재화"]
    C -->|"궁극"| D["범용 시뮬레이터<br/>뉴턴 역학 수준"]
    style A fill:#ff9999
    style B fill:#ffcc66
    style C fill:#99ccff
    style D fill:#99ff99
...
> 📊 **그림 4**: World Model 응용 분야와 대표 시스템

```mermaid
mindmap
    root((World Model<br/>응용))
        자율주행
            NVIDIA Cosmos
            Wayve GAIA-2
            합성 데이터 생성
