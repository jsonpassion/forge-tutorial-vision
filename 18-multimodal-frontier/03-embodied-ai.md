...
> 📊 **그림 1**: Embodied AI의 인식-인지-행동 루프

```mermaid
flowchart LR
    A["환경 관찰<br/>(Perception)"] --> B["상황 이해<br/>(Cognition)"]
    B --> C["목표 설정"]
    C --> D["행동 계획"]
    D --> E["행동 실행<br/>(Action)"]
    E --> F["피드백 수신"]
...
> 📊 **그림 2**: VLA 모델의 멀티모달 처리 구조

```mermaid
flowchart TD
    V["카메라 영상"] --> VE["비전 인코더<br/>(ViT)"]
    L["언어 명령"] --> TE["텍스트 인코더"]
    VE --> F["멀티모달 융합<br/>(LLM 기반)"]
    TE --> F
    F --> AD["행동 디코더"]
    AD --> R["로봇 제어<br/>(x, y, z, roll, pitch, yaw, gripper)"]
```
...
> 📊 **그림 3**: RT-2의 웹 지식 전이 과정

```mermaid
flowchart LR
    A["인터넷 데이터<br/>(이미지+텍스트)"] --> B["거대 VLM 사전학습<br/>(PaLI-X / PaLM-E)"]
    C["로봇 데모 데이터"] --> D["로봇 행동 파인튜닝"]
    B --> D
    D --> E["RT-2 모델"]
    E --> F["이미지 + 텍스트 명령"]
    F --> G["행동 토큰 출력<br/>(이산화된 로봇 제어값)"]
```
...
> 📊 **그림 5**: 로봇 학습 패러다임의 발전과 결합

```mermaid
flowchart TD
    IL["모방 학습<br/>(Imitation Learning)"] --> |"전문가 시연"| D1["상태-행동 쌍 학습"]
    RL["강화 학습<br/>(Reinforcement Learning)"] --> |"시행착오"| D2["보상 최대화"]
    FM["Foundation Model<br/>(VLM 사전학습)"] --> |"웹 데이터"| D3["상식 + 일반화"]
    D1 --> TR["현재 트렌드:<br/>모방학습 + Foundation Model"]
    D3 --> TR
    D2 -.-> |"RL 파인튜닝"| TR
    TR --> GOAL["소량 시연 + 강력한 일반화"]
```
