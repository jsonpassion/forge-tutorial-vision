...
> 📊 **그림 1**: 멀티모달 AI의 3세대 진화

```mermaid
flowchart LR
    subgraph G1["1세대: 전문 모델"]
        A1["CNN<br/>이미지"] ~~~ A2["BERT/GPT<br/>텍스트"] ~~~ A3["Wav2Vec<br/>오디오"]
    end
    subgraph G2["2세대: 브릿지 연결"]
        B1["비전 인코더"] -->|"브릿지"| B2["LLM"]
        B3["오디오 인코더"] -->|"변환"| B2
    end
    subgraph G3["3세대: 네이티브 통합"]
        C1["단일 통합 모델<br/>GPT-4o, Gemini 2.0"]
    end
    G1 -->|"진화"| G2 -->|"진화"| G3
```
...
> 📊 **그림 2**: GPT-4o의 통합 아키텍처 (추정)

```mermaid
flowchart TD
    I1["텍스트 입력"] --> T["통합 토크나이저"]
    I2["이미지 패치"] --> T
    I3["오디오 스펙트로그램"] --> T
    T --> TR["Transformer Decoder<br/>단일 신경망"]
    TR --> O1["텍스트 출력 헤드"]
    TR --> O2["오디오 출력 헤드"]
    TR --> O3["이미지 출력 헤드"]
```
...
flowchart TD
    ...
    V["비디오"] --> TK
    TK --> EMB["공통 임베딩 공간"]
    EMB --> TRANS["Transformer<br/>Cross-modal Attention"]
    TRANS --> DT["텍스트 디코더"]
    TRANS --> DI["이미지 디코더"]
    TRANS --> DA["오디오 디코더"]
    DT --> OT["텍스트 응답"]
...
> 📊 **그림 4**: 기존 3단계 파이프라인 vs 통합 End-to-End 방식
...
sequenceDiagram
    ...
    participant UM as 통합 모델
    Note over U, TTS: 기존 방식 (2-5초 지연)
    U->>ASR: 음성 입력
    ASR->>LLM: 텍스트 변환
    LLM->>TTS: 텍스트 응답
    TTS->>U: 음성 출력
    Note over U, UM: 통합 방식 (200-500ms)
    U->>UM: 음성 입력
    UM->>U: 음성 출력 (직접)
```
...
graph LR
    ...
    subgraph Gemini["Gemini 2.0"]
        GE_IN["입력: 텍스트, 이미지,<br/>오디오, 비디오"]
        GE_OUT["출력: 텍스트, 이미지, 오디오"]
    end
    subgraph Open["오픈소스 (Qwen2-VL)"]
        O_IN["입력: 텍스트, 이미지, 비디오"]
        O_OUT["출력: 텍스트"]
    end
```
