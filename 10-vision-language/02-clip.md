...
CLIP은 이 한계를 극복한 모델입니다. 이미지와 텍스트를 같은 공간에 배치하는 법을 배웠기 때문에, **어떤 텍스트 설명이든** 가장 잘 맞는 이미지를 찾아낼 수 있죠. 이 능력 덕분에 CLIP은 이후 등장한 거의 모든 Vision-Language 모델의 **비전 인코더**로 채택되었습니다. Stable Diffusion, DALL-E, LLaVA 같은 생성/이해 모델뿐 아니라, [Grounding DINO](../07-object-detection/05-detr.md)는 CLIP의 텍스트-이미지 정렬 능력을 활용해 **텍스트로 객체를 탐지**하고, [Grounded SAM](../08-segmentation/04-sam.md)은 여기에 SAM을 결합해 **텍스트로 세그멘테이션**까지 수행합니다. CLIP이 비전 AI 전반의 "눈" 역할을 하고 있는 셈이죠.
...
> 📊 **그림 1**: CLIP 듀얼 인코더 아키텍처 — 이미지와 텍스트를 독립적으로 인코딩한 뒤 공유 임베딩 공간에서 유사도를 계산합니다

```mermaid
flowchart LR
    subgraph 이미지["이미지 인코더"]
        A["입력 이미지"] --> B["ViT-L/14"]
        B --> C["이미지 임베딩"]
    end
    subgraph 텍스트["텍스트 인코더"]
        D["입력 텍스트"] --> E["Transformer"]
        E --> F["텍스트 임베딩"]
    end
    C --> G["투영 레이어"]
    F --> H["투영 레이어"]
    G --> I["공유 임베딩 공간<br/>512차원"]
    H --> I
    I --> J["코사인 유사도 계산"]
```
...
> 📊 **그림 2**: 대조 학습의 유사도 행렬 — 배치 내 N개 쌍에서 대각선(맞는 쌍)은 유사도를 높이고, 나머지(안 맞는 쌍)는 낮춥니다

```mermaid
flowchart TD
    subgraph 배치["배치 N개 쌍"]
        I1["이미지 1"] ---|맞는 쌍| T1["텍스트 1"]
        I2["이미지 2"] ---|맞는 쌍| T2["텍스트 2"]
        I3["이미지 3"] ---|맞는 쌍| T3["텍스트 3"]
    end
    I1 -.-x|안 맞는 쌍| T2
    I1 -.-x|안 맞는 쌍| T3
    I2 -.-x|안 맞는 쌍| T1
    ...
    I3 -.-x|안 맞는 쌍| T1
    I3 -.-x|안 맞는 쌍| T2
    배치 --> L["대조 손실 계산<br/>맞는 쌍: 유사도 최대화<br/>안 맞는 쌍: 유사도 최소화"]
```
...
> 📊 **그림 3**: Zero-Shot 분류 흐름 — 카테고리를 텍스트 프롬프트로 변환하여 이미지와 유사도를 비교합니다

```mermaid
sequenceDiagram
    participant U as 사용자
    participant IE as 이미지 인코더
    participant TE as 텍스트 인코더
    participant S as 유사도 계산
    U->>IE: 분류할 이미지 입력
    U->>TE: 카테고리 텍스트 입력<br/>('a photo of a cat' 등)
    IE->>S: 이미지 임베딩
    TE->>S: 텍스트 임베딩 N개
    S->>S: 코사인 유사도 계산
    S->>U: 가장 높은 유사도의<br/>카테고리를 예측 결과로 반환
```
...
> 📊 **그림 4**: CLIP 계열 모델의 발전 흐름

```mermaid
flowchart TD
    A["CLIP (OpenAI, 2021)<br/>대조 학습 + 듀얼 인코더"] --> B["OpenCLIP (LAION)<br/>오픈소스 재구현"]
    A --> C["SigLIP (Google)<br/>Sigmoid 손실 함수"]
    A --> D["EVA-CLIP (BAAI)<br/>강화된 비전 인코더"]
    A --> E["MetaCLIP (Meta)<br/>데이터 큐레이션 개선"]
    C --> F["SigLIP 2 (Google, 2025)<br/>다국어 + 지역 이해"]
    A -->|비전 인코더로 채택| G["Stable Diffusion<br/>DALL-E<br/>LLaVA 등"]
```
