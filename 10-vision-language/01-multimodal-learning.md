...
> 📊 **그림 1**: 멀티모달 학습 — 다양한 모달리티를 하나의 공간으로 연결

```mermaid
graph TD
    T["텍스트<br/>BERT, GPT"] --> E["공유 임베딩 공간<br/>Shared Embedding Space"]
    I["이미지<br/>ViT, ResNet"] --> E
    A["오디오<br/>Whisper"] --> E
    V["비디오<br/>VideoMAE"] --> E
    E --> U["멀티모달 이해<br/>검색, 생성, 추론"]
```
...
> 📊 **그림 2**: 시각-언어 연결의 세 가지 패러다임 비교

```mermaid
flowchart LR
    subgraph C["대조 학습<br/>Contrastive"]
        C1["이미지"] -. "같으면 가깝게" .- C2["텍스트"]
    end
    subgraph G["생성 학습<br/>Generative"]
        G1["이미지"] --> G2["텍스트 생성"]
        G3["텍스트"] --> G4["이미지 생성"]
    end
    subgraph F["융합 학습<br/>Fusion"]
        F1["이미지"] --> F3["크로스 어텐션"]
        F2["텍스트"] --> F3
        F3 --> F4["통합 이해"]
    end
...
이 구조가 왜 강력하냐면, 각 블록에 이미 잘 학습된 모델을 가져다 쓸 수 있기 때문입니다. [ViT](../09-vision-transformer/03-vit.md)는 수억 장의 이미지로, LLM은 수조 개의 토큰으로 이미 사전학습이 되어 있으니, 브릿지 모듈만 새로 학습하면 되는 거죠. 그리고 이 비전 인코더들은 우리가 앞서 배운 [객체 탐지](../07-object-detection/01-detection-basics.md), [세그멘테이션](../08-segmentation/01-semantic-segmentation.md)에서도 백본으로 활용되는 바로 그 모델들입니다. 이 접근법 덕분에 상대적으로 적은 비용으로 강력한 멀티모달 모델을 만들 수 있게 되었습니다.

> 📊 **그림 3**: Vision-Language 모델(VLM)의 기본 아키텍처

```mermaid
flowchart LR
    IMG["입력 이미지"] --> VE["비전 인코더<br/>ViT, SigLIP"]
    VE --> |"이미지 임베딩"| BR["브릿지 모듈<br/>Q-Former,<br/>Linear Projection"]
    BR --> |"변환된 토큰"| LLM["언어 모델(LLM)<br/>LLaMA, GPT-4"]
    TXT["텍스트 입력"] --> LLM
    LLM --> OUT["텍스트 응답"]
```
...
> 📊 **그림 5**: 패러다임 전환 — 모든 모달리티를 LLM의 토큰으로 변환

```mermaid
flowchart TD
    subgraph OLD["기존 접근법"]
        O1["이미지 모델"] --- O3["별도의 융합 레이어"]
        O2["텍스트 모델"] --- O3
    end
    subgraph NEW["최신 접근법"]
        N1["이미지"] --> N2["비전 인코더"]
