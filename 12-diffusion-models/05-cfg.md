...
> 📊 **그림 1**: Classifier Guidance vs Classifier-Free Guidance 비교

```mermaid
flowchart LR
    subgraph CG["Classifier Guidance"]
        A1["Diffusion 모델"] --> B1["노이즈 예측"]
        C1["별도 분류기"] --> D1["그래디언트 계산"]
        B1 --> E1["가이드된 예측"]
        D1 --> E1
    end
...
> 📊 **그림 2**: CFG 추론 과정 — 두 예측의 차이를 증폭

```mermaid
flowchart TD
    X["노이즈 이미지 x_t"] --> UC["비조건부 예측<br/>조건 = 빈 텍스트"]
    X --> CO["조건부 예측<br/>조건 = 텍스트 프롬프트"]
    UC --> DIFF["차이 계산<br/>조건부 - 비조건부"]
    CO --> DIFF
    DIFF --> AMP["w배 증폭<br/>가이던스 스케일 적용"]
    UC --> ADD["최종 노이즈 예측"]
    AMP --> ADD
    ADD --> NEXT["다음 디노이징 스텝"]
```
...
> 📊 **그림 3**: CFG 학습 시 조건 드롭아웃 과정

```mermaid
flowchart TD
    DATA["학습 데이터<br/>이미지 + 텍스트 조건"] --> RNG{"랜덤 확률"}
    RNG -->|"80~90%"| COND["조건 유지<br/>텍스트 임베딩 입력"]
    RNG -->|"10~20%"| DROP["조건 드롭아웃<br/>빈 임베딩으로 대체"]
    COND --> MODEL["U-Net 학습"]
    DROP --> MODEL
    MODEL --> RESULT["하나의 모델이<br/>조건부 + 비조건부<br/>모두 가능"]
```
...
> 📊 **그림 4**: 네거티브 프롬프트의 작동 원리

```mermaid
flowchart LR
    X["노이즈 이미지"] --> NEG["네거티브 예측<br/>'blurry, low quality'"]
    X --> POS["포지티브 예측<br/>'beautiful cat'"]
    NEG --> DIR["방향 계산<br/>포지티브 - 네거티브"]
    POS --> DIR
    DIR --> SCALE["w배 증폭"]
    SCALE --> FINAL["최종 예측<br/>원하는 방향으로 이동<br/>원치 않는 방향에서 이탈"]
```
