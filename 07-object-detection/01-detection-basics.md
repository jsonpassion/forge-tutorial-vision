...
> 📊 **그림 1**: 컴퓨터 비전 태스크의 발전 단계

```mermaid
flowchart LR
    A["이미지 분류<br/>무엇인가?"] --> B["객체 탐지<br/>무엇이 어디에?"] --> C["인스턴스 분할<br/>정확한 윤곽선"]
    A -.- A1["라벨 1개"] 
    B -.- B1["클래스 + 박스 x N"]
    C -.- C1["클래스 + 마스크 x N"]
```
...
```mermaid
flowchart TD
    A["모든 예측 박스를<br/>신뢰도 기준 정렬"] --> B["가장 높은 점수의<br/>박스 선택"]
    B --> C["최종 결과에 추가"]
    C --> D{"나머지 박스와<br/>IoU 계산"}
    D --> E["IoU >= 임계값인<br/>박스 제거"]
    E --> F{"남은 박스가<br/>있는가?"}
    F -- 예 --> B
    F -- 아니오 --> G["최종 결과 반환"]
```
...
flowchart LR
    ...
    C --> F["매칭 안 된 정답"]
    F --> G["FN"]
    D --> H["Precision<br/>TP / (TP+FP)"]
    E --> H
    D --> I["Recall<br/>TP / (TP+FN)"]
    G --> I
    H --> J["PR 곡선 면적"]
    I --> J
    J --> K["AP"]
    K --> L["mAP<br/>클래스별 AP 평균"]
```
...
> 📊 **그림 4**: Two-Stage vs One-Stage 탐지기 구조 비교

```mermaid
flowchart TD
    subgraph TWO["Two-Stage Detector"]
        direction LR
        T1["입력 이미지"] --> T2["백본 CNN"]
        T2 --> T3["Region Proposal<br/>후보 영역 추출"]
        T3 --> T4["분류 + 박스 정밀 조정"]
        T4 --> T5["최종 결과"]
    end
    subgraph ONE["One-Stage Detector"]
        direction LR
        O1["입력 이미지"] --> O2["백본 CNN"]
        O2 --> O3["그리드별<br/>클래스 + 위치 예측"]
        O3 --> O4["최종 결과"]
    end
```
...
> 📊 **그림 5**: 객체 탐지 기술의 발전 타임라인

```mermaid
flowchart LR
    subgraph ERA1["전통적 방법<br/>2001-2012"]
        A1["Viola-Jones<br/>2001"] --> A2["HOG+SVM<br/>2005"] --> A3["DPM<br/>2008"]
    end
    subgraph ERA2["CNN 기반<br/>2012-2020"]
        B1["R-CNN<br/>2014"] --> B2["Faster R-CNN<br/>2015"] --> B3["YOLO / SSD<br/>2016"]
    end
    subgraph ERA3["Transformer<br/>2020-현재"]
        C1["DETR<br/>2020"] --> C2["최신 모델들"]
    end
    ERA1 --> ERA2 --> ERA3
```
