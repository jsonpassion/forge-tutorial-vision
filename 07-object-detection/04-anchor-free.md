...
> 📊 **그림 1**: CornerNet의 탐지 파이프라인 — 두 코너의 키포인트를 찾아 매칭

```mermaid
flowchart LR
    A["입력 이미지"] --> B["Hourglass<br/>백본"]
    B --> C["Top-Left<br/>히트맵"]
    B --> D["Bottom-Right<br/>히트맵"]
    B --> E["Embedding<br/>벡터"]
    C --> F["코너 매칭"]
    D --> F
    E --> F
...
> 📊 **그림 2**: CenterNet의 탐지 파이프라인 — 중심점 하나로 객체를 정의

```mermaid
flowchart LR
    A["입력 이미지"] --> B["백본<br/>ResNet/DLA"]
    B --> C["Center 히트맵<br/>클래스별 중심점"]
    B --> D["크기 회귀<br/>w, h"]
    B --> E["오프셋 보정<br/>dx, dy"]
    C --> F["피크 추출<br/>NMS 불필요"]
    D --> F
    E --> F
    F --> G["최종 탐지 결과"]
```
...
> 📊 **그림 3**: FCOS의 픽셀별 예측 구조 — FPN 멀티스케일 + Center-ness
...
> 📊 **그림 4**: 세 가지 Anchor-Free 탐지기의 접근 방식 비교

```mermaid
graph TD
    ROOT["Anchor-Free 탐지"] --> KP["키포인트 기반"]
    ROOT --> DP["밀집 예측 기반"]
    KP --> CN["CornerNet<br/>코너 2개 매칭"]
    KP --> CT["CenterNet<br/>중심점 1개"]
    DP --> FC["FCOS<br/>픽셀별 4방향 거리"]
    CN -. "2018, AP 42.2%" .-> CN
    CT -. "2019, AP 45.1%" .-> CT
    FC -. "2019, AP 44.7%" .-> FC
```
...
> 📊 **그림 5**: Anchor-Free 탐지기의 두 갈래와 대표 모델들

```mermaid
flowchart LR
    A["Anchor-Free"] --> B["키포인트 기반<br/>Keypoint-based"]
    A --> C["밀집 예측 기반<br/>Dense Prediction"]
    B --> B1["CornerNet<br/>코너 쌍"]
    B --> B2["CenterNet<br/>중심점"]
    B --> B3["ExtremeNet<br/>극단점"]
    C --> C1["FCOS<br/>거리 예측"]
    C --> C2["FoveaBox<br/>중심 영역"]
    B --> D["융합"]
    C --> D
    D --> E["YOLOv8 / YOLO11"]
```
