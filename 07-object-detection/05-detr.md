...
> 📊 **그림 1**: DETR의 3대 구성 요소와 데이터 흐름

```mermaid
flowchart LR
    A["입력 이미지"] --> B["CNN 백본<br/>(ResNet-50)"]
    B --> C["Transformer<br/>인코더"]
    C --> D["Transformer<br/>디코더"]
    E["Object Query<br/>(N=100)"] --> D
    D --> F["FFN"]
    F --> G["클래스 + 박스"]
    F --> H["no-object"]
```
...
> 📊 **그림 2**: 헝가리안 매칭 — 예측과 정답의 1:1 최적 매칭

```mermaid
graph LR
    subgraph 예측["예측 슬롯 (N=100)"]
        P1["예측 1"]
        P2["예측 2"]
        P3["예측 3"]
        P4["..."]
...
> 📊 **그림 3**: DETR 전체 파이프라인 — 4단계 처리 흐름

```mermaid
flowchart TD
    A["입력 이미지"] --> B["Step 1: CNN 백본<br/>특징 맵 추출"]
    B --> B2["+ Positional Encoding"]
    B2 --> C["Step 2: Transformer 인코더<br/>전역 문맥 파악<br/>(Self-Attention)"]
    C --> D["Step 3: Transformer 디코더<br/>Object Query x N<br/>(Cross-Attention)"]
    D --> E["Step 4: FFN 예측 헤드"]
    E --> F["(클래스, 박스)"]
    E --> G["no-object"]
    H["헝가리안 매칭<br/>(학습 시)"] -.->|"1:1 매칭"| E
```
...
> 📊 **그림 4**: DETR 계열 모델의 진화 계보

```mermaid
graph TD
    A["DETR (2020)<br/>최초의 Transformer 탐지기"] --> B["Deformable DETR (2021)<br/>변형 가능 어텐션"]
    B --> C["DN-DETR (2022)<br/>디노이징 학습"]
    B --> D["DAB-DETR (2022)<br/>앵커 박스 쿼리"]
    C --> E["DINO (2022)<br/>AP 49.4%"]
    D --> E
    E --> F["RT-DETR (2024)<br/>실시간 달성, 114 FPS"]
    F --> G["RT-DETRv2 (2024)<br/>AP 54.3%"]
