...
> 📊 **그림 1**: R-CNN(2-stage) vs YOLO(1-stage) 탐지 방식 비교

```mermaid
flowchart LR
    subgraph two["2-Stage: R-CNN 계열"]
        A1["입력 이미지"] --> B1["후보 영역 추출<br/>약 2000개"]
        B1 --> C1["각 영역 분류<br/>+ 박스 보정"]
        C1 --> D1["최종 결과"]
    end
    subgraph one["1-Stage: YOLO"]
        A2["입력 이미지"] --> B2["CNN 순전파<br/>단 1회"]
        B2 --> C2["그리드별 예측<br/>클래스 + 박스 동시"]
        C2 --> D2["최종 결과"]
    end
```
...
> 📊 **그림 2**: YOLOv1 그리드 기반 탐지 파이프라인

```mermaid
flowchart TD
    A["입력 이미지"] --> B["S x S 그리드 분할<br/>기본 7x7"]
    B --> C["각 셀이 B개 박스 예측<br/>x, y, w, h, confidence"]
    B --> D["각 셀이 C개 클래스<br/>확률 예측"]
    C --> E["출력 텐서<br/>S x S x (B x 5 + C)"]
    D --> E
    E --> F["NMS 후처리"]
    F --> G["최종 탐지 결과"]
```
...
> 📊 **그림 3**: YOLO 버전별 핵심 진화 흐름

```mermaid
flowchart LR
    V1["v1 (2016)<br/>1-stage 탐지 시작"] --> V2["v2 (2016)<br/>앵커 박스 도입"]
    V2 --> V3["v3 (2018)<br/>멀티스케일 예측"]
    V3 --> V4["v4 (2020)<br/>Mosaic 증강"]
    V4 --> V5["v5 (2020)<br/>PyTorch 전환"]
    V5 --> V8["v8 (2023)<br/>Anchor-Free 전환"]
    V8 --> V11["v11 (2024)<br/>C2PSA 어텐션"]
...
```mermaid
flowchart TD
    A["입력 이미지"] --> B["Backbone<br/>CSPDarknet + C2f"]
    B --> C1["P3 특징맵<br/>소형 객체"]
    B --> C2["P4 특징맵<br/>중형 객체"]
    B --> C3["P5 특징맵<br/>대형 객체"]
    C1 --> D["Neck<br/>PAN-FPN 구조"]
    C2 --> D
    C3 --> D
    D --> E1["분류 헤드<br/>Decoupled"]
    D --> E2["박스 회귀 헤드<br/>Anchor-Free"]
    E1 --> F["최종 탐지 결과"]
    E2 --> F
```
...
> 📊 **그림 5**: Mosaic 데이터 증강 개념도

```mermaid
flowchart TD
    A1["이미지 A"] --> M["Mosaic 합성"]
    A2["이미지 B"] --> M
    A3["이미지 C"] --> M
    A4["이미지 D"] --> M
    M --> R["합성 이미지 1장<br/>4장의 정보 포함"]
    R --> E1["배경 다양성 증가"]
    R --> E2["작은 객체 학습 유리"]
    R --> E3["배치 크기 4배 효과"]
```
