...
> 📊 **그림 1**: 형태학적 연산의 전체 구조 — 기본 연산과 복합 연산의 관계

```mermaid
graph TD
    SE["구조 요소<br/>Structuring Element"] --> E["침식<br/>Erosion"]
    SE --> D["팽창<br/>Dilation"]
    E --> OP["열기 Opening<br/>침식 + 팽창"]
    D --> OP
    D --> CL["닫기 Closing<br/>팽창 + 침식"]
    E --> CL
    E --> GR["그래디언트<br/>팽창 - 침식"]
    D --> GR
    OP --> TH["Top Hat<br/>원본 - 열기"]
    CL --> BH["Black Hat<br/>닫기 - 원본"]
```
...
> 📊 **그림 2**: 열기(Opening) 연산의 처리 흐름 — 침식으로 노이즈를 제거한 뒤 팽창으로 크기 복원

```mermaid
flowchart LR
    A["원본 이미지<br/>노이즈 포함"] --> B["1단계: 침식<br/>작은 점 제거<br/>물체도 축소"]
    B --> C["2단계: 팽창<br/>물체 크기 복원<br/>노이즈는 이미 사라짐"]
    C --> D["결과<br/>노이즈 제거 + 크기 유지"]
```
...
> 📊 **그림 3**: 닫기(Closing) 연산의 처리 흐름 — 팽창으로 구멍을 메운 뒤 침식으로 크기 복원

```mermaid
flowchart LR
    A["원본 이미지<br/>내부 구멍 있음"] --> B["1단계: 팽창<br/>구멍 메움<br/>물체도 확장"]
    B --> C["2단계: 침식<br/>물체 크기 복원<br/>구멍은 이미 메워짐"]
    C --> D["결과<br/>구멍 제거 + 크기 유지"]
```
...
> 📊 **그림 4**: 이진화 + 형태학 연산 전처리 파이프라인

```mermaid
flowchart TD
    A["원본 이미지"] --> B["흑백 변환<br/>IMREAD_GRAYSCALE"]
    B --> C["이진화<br/>threshold 127"]
    C --> D["열기 Opening<br/>작은 노이즈 제거"]
    D --> E["닫기 Closing<br/>내부 구멍 메우기"]
    E --> F["정리된 이진 이미지"]
```
...
> 💡 **알고 계셨나요?**: **수학적 형태학(Mathematical Morphology)**의 탄생에는 의외의 배경이 있습니다. 1964년 프랑스 파리의 **에콜 데 민 드 파리(Ecole des Mines de Paris)**에서 Georges Matheron과 Jean Serra가 **철광석 분석** 문제를 풀기 위해 이 이론을 개발했거든요. 광석 표본 이미지에서 철 입자의 크기와 분포를 자동으로 분석하려면, 이미지 속 형태를 수학적으로 다루는 도구가 필요했던 거죠. 광산 지질학(Mining Geology)에서 시작된 기법이 오늘날 의료 영상, 자율 주행, 반도체 검사까지 쓰이고 있다니 놀랍지 않나요?

"Mathematical Morphology(수학적 형태학)"이라는 이름 자체도 재미있는 사연이 있습니다. 1966년 겨울, 프랑스 낭시(Nancy)의 한 **펍(pub)**에서 Matheron과 Serra가 맥주를 마시며 자신들의 이론에 이름을 붙이기 위해 고민했다고 해요. "형태를 수학적으로 다룬다"는 뜻에서 "Mathematical Morphology"로 결정했는데, 펍에서 탄생한 이름이 학문 분야의 정식 명칭이 된 셈이죠!

이 이론의 핵심 아이디어는 단순하면서도 강력합니다. **알려진 형태(구조 요소)로 미지의 형태(이미지)를 탐침(probe)한다**는 것인데요, 이 철학이 침식, 팽창, 열기, 닫기 같은 모든 형태학 연산의 근간을 이루고 있습니다.

## 흔한 오해와 팁

> ⚠️ **흔한 오해**: "형태학 연산은 이진 이미지에만 쓸 수 있다" — 아닙니다! **그레이스케일 이미지에서도 동작**합니다. 이진에서의 AND/OR 연산이 그레이스케일에서는 **min/max 연산**으로 대체되거든요. 침식은 커널 범위 내 최솟값(min), 팽창은 최댓값(max)을 취합니다. 그레이스케일 형태학은 배경 조명 보정, 텍스처 분석 같은 곳에 유용하게 쓰여요.

> ⚠️ **흔한 오해**: "Opening과 Closing을 둘 다 적용하면 원본으로 돌아온다" — **돌아오지 않습니다!** 열기(침식 → 팽창)와 닫기(팽창 → 침식)는 모두 **비가역적(irreversible)** 연산이에요. 열기에서 깎여나간 작은 구조는 팽창으로 복원되지 않고, 닫기에서 메워진 구멍은 침식으로 다시 생기지 않습니다. 한번 손실된 정보는 되돌릴 수 없다는 점을 기억하세요.

> 🔥 **실무 팁**: 구조 요소의 **모양 선택이 결과에 큰 영향**을 줍니다! **원형(MORPH_ELLIPSE)**은 모든 방향에 균일하게 작용하는 등방향 연산에 적합하고, **사각형(MORPH_RECT)**은 수평/수직 방향성이 있는 특징을 다룰 때 좋습니다. **십자형(MORPH_CROSS)**은 4-연결성 기반의 연결 분석에 유용하고요. 기본값으로 사각형을 쓰는 경우가 많은데, 원형 물체를 다룬다면 타원형 구조 요소로 바꿔보세요. 결과가 눈에 띄게 달라지거든요.
