...
> 📊 **그림 1**: 커널(필터)의 합성곱 연산 과정

```mermaid
flowchart LR
    A["입력 이미지"] --> B["커널을 좌상단에 배치"]
    B --> C["겹치는 픽셀과<br/>커널 값을 곱셈"]
    C --> D["곱한 값을 모두 합산"]
    D --> E["결과를 새 픽셀에 기록"]
    E --> F{"마지막 위치?"}
    F -- "아니오" --> G["커널을 한 칸 이동"]
...
> 📊 **그림 2**: 블러 필터 종류별 특성 비교

```mermaid
graph TD
    A["블러 필터"] --> B["평균 블러<br/>Box Blur"]
    A --> C["가우시안 블러<br/>Gaussian"]
    A --> D["미디언 블러<br/>Median"]
    A --> E["양방향 필터<br/>Bilateral"]
    B --> B1["단순 평균<br/>빠름, 에지 손상"]
    C --> C1["중심 가중치 높음<br/>자연스러운 결과"]
    D --> D1["중앙값 사용<br/>소금-후추 노이즈 제거"]
    E --> E1["에지 보존<br/>느리지만 고품질"]
```
...
> 📊 **그림 4**: 커널 크기에 따른 효과 변화

```mermaid
flowchart LR
    A["3x3 커널<br/>9 픽셀 참조"] --> B["5x5 커널<br/>25 픽셀 참조"]
    B --> C["7x7 커널<br/>49 픽셀 참조"]
    C --> D["11x11 커널<br/>121 픽셀 참조"]
    style A fill:#e8f5e9
    style B fill:#fff9c4
    style C fill:#ffe0b2
    style D fill:#ffcdd2
