...
> 📊 **그림 1**: 에지 검출의 기본 원리 — 밝기 변화와 기울기

```mermaid
flowchart LR
    A["밝은 영역<br/>밝기 = 200"] --> B["경계<br/>밝기 급변"]
    B --> C["어두운 영역<br/>밝기 = 50"]
    B --> D["기울기 계산<br/>1차 미분"]
    D --> E{"기울기 크기"}
    E -->|"큼"| F["에지 O"]
...
> 📊 **그림 2**: Sobel 에지 검출 처리 흐름

```mermaid
flowchart LR
    A["입력 이미지"] --> B["Sobel Gx<br/>수평 기울기"]
    A --> C["Sobel Gy<br/>수직 기울기"]
    B --> D["크기 합성<br/>sqrt(Gx2 + Gy2)"]
    C --> D
    D --> E["에지 맵"]
```
...
> 📊 **그림 3**: Canny 에지 검출 4단계 파이프라인

```mermaid
flowchart TD
    A["입력 이미지"] --> B["1. 가우시안 블러<br/>노이즈 제거"]
    B --> C["2. Sobel 기울기 계산<br/>방향 + 크기"]
    C --> D["3. 비최대 억제 NMS<br/>1픽셀 두께로 정리"]
    D --> E{"4. 이중 임계값"}
    E -->|"강한 에지"| F["확정 에지"]
    E -->|"약한 에지"| G{"강한 에지와<br/>연결?"}
    G -->|"연결됨"| F
    G -->|"고립됨"| H["제거"]
    F --> I["최종 에지 맵"]
```
...
> 📊 **그림 4**: 세 가지 에지 검출 방법의 관계와 특성

```mermaid
graph TD
    A["에지 검출"] --> B["Sobel<br/>1차 미분"]
    A --> C["Laplacian<br/>2차 미분"]
    A --> D["Canny<br/>멀티스테이지"]
    B --> B1["방향별 기울기 제공<br/>Gx, Gy 분리"]
    C --> C1["모든 방향 한번에<br/>노이즈 민감"]
    D --> D1["Sobel 내부 사용"]
    D --> D2["NMS + 이중 임계값"]
    D --> D3["가장 깨끗한 결과"]
```
