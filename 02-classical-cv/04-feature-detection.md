...
> 📊 **그림 1**: 특징점 검출과 매칭의 전체 흐름

```mermaid
flowchart LR
    A["이미지 입력"] --> B["특징점 검출<br/>Keypoint Detection"]
    B --> C["기술자 추출<br/>Descriptor Extraction"]
    C --> D["기술자 비교<br/>Matching"]
    D --> E["대응점 쌍"]
```
...
> 📊 **그림 2**: SIFT 알고리즘의 4단계 처리 과정

```mermaid
flowchart TD
    A["원본 이미지"] --> B["1. 스케일 공간 구축<br/>가우시안 블러 피라미드"]
    B --> C["2. 극값 검출<br/>DoG에서 극대/극소 탐색"]
    C --> D["3. 방향 할당<br/>주요 기울기 방향 계산"]
    D --> E["4. 기술자 생성<br/>16x16 영역 -> 4x4 블록 x 8방향"]
    E --> F["128차원 float 벡터"]
```
...
> 📊 **그림 3**: ORB의 구성 — FAST + Oriented BRIEF

```mermaid
flowchart TD
    A["원본 이미지"] --> B["FAST 검출기<br/>코너 기반 특징점 탐색"]
    A --> C["이미지 피라미드<br/>멀티스케일 처리"]
    C --> B
    B --> D["방향 계산<br/>Intensity Centroid"]
    D --> E["Rotated BRIEF<br/>방향 보정된 바이너리 기술자"]
    E --> F["32차원 바이너리 벡터"]
```
...
flowchart LR
    ...
    B1 --> M["매처<br/>BFMatcher / FLANN"]
    B2 --> M
    M --> R["비율 테스트<br/>Lowe's Ratio Test"]
    R --> G["좋은 매칭만 선별"]
```
