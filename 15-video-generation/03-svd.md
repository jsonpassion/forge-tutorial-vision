...
> 📊 **그림 1**: Image-to-Video 생성 흐름 — 입력 이미지가 비디오로 변환되는 과정

```mermaid
flowchart LR
    A["입력 이미지"] --> B["VAE 인코딩"]
    B --> C["첫 프레임 Latent"]
    C --> D["3D U-Net<br/>노이즈 제거"]
    E["랜덤 노이즈<br/>(나머지 프레임)"] --> D
    F["Motion Bucket<br/>FPS 조건"] --> D
    D --> G["VAE 디코딩"]
    G --> H["비디오 프레임들"]
```
...
> 📊 **그림 2**: SD vs SVD 블록 비교 — Temporal 레이어가 추가된 구조

```mermaid
flowchart TD
    subgraph SD["기존 Stable Diffusion 블록"]
        direction LR
        S1["2D Conv"] --> S2["Spatial<br/>Attention"] --> S3["Cross<br/>Attention"]
    end
    subgraph SVD["SVD 블록 (시간 축 확장)"]
        direction LR
        V1["2D Conv"] --> V2["Temporal<br/>Conv"] --> V3["Spatial<br/>Attention"] --> V4["Temporal<br/>Attention"] --> V5["Cross<br/>Attention"]
    end
    SD -.->|"시간 레이어 추가"| SVD
```
...
> 📊 **그림 3**: SVD 조건 주입 — 입력 이미지가 3가지 경로로 모델에 반영되는 구조

```mermaid
flowchart TD
    IMG["입력 이미지"] --> P1["VAE 인코더"]
    IMG --> P2["CLIP 이미지 인코더"]
    IMG --> P3["노이즈 마스크 생성"]
    P1 --> L["Latent 연결<br/>첫 프레임에 직접 결합"]
    P2 --> C["Cross Attention<br/>시각적 특징 주입"]
    P3 --> M["노이즈 마스킹<br/>첫 프레임 노이즈 감소"]
    L --> UNET["3D U-Net"]
    C --> UNET
    M --> UNET
    MB["Motion Bucket ID"] --> UNET
    FPS["FPS 조건"] --> UNET
```
...
> 📊 **그림 4**: SVD 3단계 훈련 파이프라인

```mermaid
flowchart LR
    subgraph S1["Stage 1"]
        A1["대규모 이미지<br/>데이터셋"] --> B1["SD 이미지<br/>사전 훈련"]
    end
    subgraph S2["Stage 2"]
        A2["LVD-142M<br/>비디오 데이터셋"] --> B2["비디오<br/>사전 훈련"]
    end
    subgraph S3["Stage 3"]
        A3["고품질 소규모<br/>데이터셋"] --> B3["미세 조정"]
    end
    S1 -->|"시각적 표현 학습"| S2
    S2 -->|"시간적 일관성 학습"| S3
    S3 --> R["SVD 최종 모델"]
...
> 📊 **그림 5**: 프로 크리에이터 워크플로우 — 이미지 생성부터 비디오 완성까지

```mermaid
flowchart LR
    A["텍스트 프롬프트"] --> B["SD / SDXL / FLUX<br/>이미지 생성"]
    B --> C["완벽한 첫 프레임"]
    C --> D["SVD<br/>애니메이션 적용"]
    D --> E["기본 비디오"]
    E --> F{"추가 제어<br/>필요?"}
    F -->|"예"| G["ControlNet +<br/>AnimateDiff"]
    F -->|"아니오"| H["프레임 보간<br/>후처리"]
    G --> H
    H --> I["최종 비디오"]
```
