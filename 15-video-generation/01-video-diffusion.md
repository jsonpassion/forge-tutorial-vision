...
> 📊 **그림 1**: 이미지 텐서와 비디오 텐서의 차원 비교

```mermaid
graph LR
    subgraph 이미지["이미지 텐서"]
        I["H x W x C<br/>높이 x 너비 x 채널"]
    end
    subgraph 비디오["비디오 텐서"]
        V["T x H x W x C<br/>프레임 x 높이 x 너비 x 채널"]
    end
    I -->|"시간 축 T 추가"| V
    style 이미지 fill:#e8f4fd,stroke:#2196F3
    style 비디오 fill:#fff3e0,stroke:#FF9800
```
...
> 📊 **그림 2**: Factorized Space-Time 처리 흐름 (3D U-Net 블록)

```mermaid
flowchart TD
    A["입력 비디오 텐서<br/>(B, T, C, H, W)"] --> B["공간 처리<br/>2D Conv (1x3x3)"]
    B --> C["각 프레임 독립 처리<br/>공간 특징 추출"]
    C --> D["시간 처리<br/>Temporal Attention"]
    D --> E["같은 위치의 프레임 간<br/>정보 교환"]
    E --> F["잔차 연결<br/>Residual Connection"]
    F --> G["출력 비디오 텐서<br/>(B, T, C', H, W)"]
```
...
> 📊 **그림 3**: Temporal Attention 작동 과정 — 프레임 간 정보 교환

```mermaid
sequenceDiagram
    participant F1 as 프레임 1
    participant F2 as 프레임 2
    participant F3 as 프레임 3
    participant F4 as 프레임 4
    Note over F1,F4: 같은 공간 위치(h, w)에서 특징 추출
    F1->>F2: Q-K 어텐션 가중치
    F1->>F3: Q-K 어텐션 가중치
    F1->>F4: Q-K 어텐션 가중치
    F2->>F1: 정보 공유 (Value)
    F3->>F1: 정보 공유 (Value)
    F4->>F1: 정보 공유 (Value)
    Note over F1,F4: 모든 프레임이 상호 참조하여 시간적 일관성 확보
```
...
> 📊 **그림 5**: 비디오 생성 핵심 과제와 해결 전략

```mermaid
graph TD
    P["비디오 생성 과제"] --> T["시간적 일관성"]
    P --> M["메모리/계산량"]
    P --> N["동작 자연스러움"]
    T --> T1["Temporal Attention"]
    T --> T2["Temporal Conv"]
    T --> T3["조건부 생성<br/>첫 프레임 고정"]
    M --> M1["Latent Space 생성"]
    M --> M2["Factorized Attention"]
    M --> M3["프레임 서브샘플링"]
    N --> N1["대규모 학습 데이터"]
    N --> N2["모션 모듈 학습"]
```
...
> 📊 **그림 4**: Latent Video Diffusion 파이프라인

```mermaid
flowchart LR
    A["텍스트 프롬프트"] --> B["텍스트 인코더<br/>CLIP"]
    B --> D
    C["랜덤 노이즈<br/>(T', H', W')"] --> D["3D U-Net<br/>반복 노이즈 제거"]
    D --> E["Latent 비디오<br/>압축된 시공간"]
    E --> F["3D VAE<br/>디코더"]
    F --> G["최종 비디오<br/>(T, H, W, C)"]
