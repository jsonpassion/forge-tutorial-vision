...
> 📊 **그림 1**: 딥러닝 프로젝트의 5단계 파이프라인

```mermaid
flowchart LR
    A["1. 데이터 준비<br/>Dataset + DataLoader"] --> B["2. 모델 정의<br/>CNN 아키텍처"]
    B --> C["3. 학습<br/>Forward + Backward"]
    C --> D["4. 평가<br/>정확도 측정"]
    D --> E["5. 저장/배포<br/>state_dict"]
    D -.->|"성능 부족"| C
```
...
> 📊 **그림 4**: 데이터 준비 흐름 — 원본에서 배치까지

```mermaid
flowchart LR
    A["MNIST 원본<br/>0-255 픽셀"] --> B["ToTensor<br/>0-1 float 변환"]
    B --> C["Normalize<br/>평균 0.13, 표준편차 0.31"]
    C --> D["Dataset<br/>60,000장"]
    D --> E["DataLoader<br/>배치 크기 64"]
    E --> F["모델 입력<br/>(64, 1, 28, 28)"]
```
...
> 📊 **그림 2**: MNISTNet CNN 아키텍처 — 입력에서 분류까지의 텐서 변환

```mermaid
flowchart TD
    A["입력 이미지<br/>1 x 28 x 28"] --> B["Conv2d + BN + ReLU<br/>32 x 28 x 28"]
    B --> C["MaxPool2d<br/>32 x 14 x 14"]
    C --> D["Conv2d + BN + ReLU<br/>64 x 14 x 14"]
    D --> E["MaxPool2d<br/>64 x 7 x 7"]
    E --> F["AdaptiveAvgPool2d<br/>64 x 1 x 1"]
    F --> G["Flatten + Dropout<br/>64"]
    G --> H["Linear<br/>10 클래스 출력"]
```
...
> 📊 **그림 3**: 학습 루프 핵심 사이클 — 매 배치마다 반복

```mermaid
flowchart LR
    A["zero_grad<br/>그래디언트 초기화"] --> B["forward<br/>순전파"]
    B --> C["loss<br/>손실 계산"]
    C --> D["backward<br/>역전파"]
    D --> E["step<br/>파라미터 업데이트"]
    E -.->|"다음 배치"| A
```
